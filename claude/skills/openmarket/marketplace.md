...
description: Discover, install, and publish OpenMarket packages from the central registry — alert recipe packs, ontology packs, event-watch packs, strategy templates, routine packs (schedules plus carried watches, installed paused; each member kind has its own arm ritual), sandboxed WRUN indicators, kScript indicator sources, and kScript libraries. Use this skill BEFORE hand-rolling an alert or indicator the marketplace may already provide, when the user asks to install a package, or when a creator wants to publish one; building a custom WRUN indicator is the wrun skill. Installs are consent-gated — the agent proposes, the user approves.
...
The registry (`registry.openmarket.xyz`) hosts installable trading intelligence in these kinds: `alert-recipe-pack`, `ontology-pack` and `event-watch-pack` (§"Alert, ontology and event-watch packs"); `strategy-template` (§"Strategy templates"); `wrun-indicator` (sandboxed; outputs become metric ids `wrun/@scope/name/output` — §"WRUN packages"); `kscript-indicator` (run by reference on charts, never installed) and `kscript-library` (publish-only, imported by kScript source) — §"kScript indicators"; `routine-pack` (schedules plus carried watches, local install until the registry learns the kind: §"Routine packs"). Search it BEFORE hand-rolling an alert or indicator.

### Guardrails

- Installs are consent-gated — the agent proposes, the user approves: `package_install` only after the user's own yes to that specific package (never set `yes` yourself on chat surfaces; over MCP `yes=true` only after that yes); a personalized template install (`overrides`, a `market` retarget) also needs the `candidate` token `package_try` minted.
...
- A routine pack lands PAUSED behind a digest-bound preview; arming is per kind: a carded `schedule_resume` per screenshot, the terminal adopt-then-resume two-step for an errand (chat and MCP refuse), `om event-watch resume` for a carried watch (§"Routine packs").
...
| "what does this routine pack do?" / "arm the EU open routine" | `package_show_routine` → `package_install` → `schedule_resume` | roster by origin, never a URL; one yes arms one screenshot member; errands arm at a terminal (adopt, then resume) |
...
## Routine packs
Schedules plus carried watches as one pack: publish `schedule:<id>`, members install paused, each kind has its own arm ritual; the registry does not accept the kind yet.

A `routine-pack` ships `routines/<member>.json` files, each a `workspace_screenshot` or `errand` schedule on one of two whens: a `cron` clock, or a `monitor` cadence with one probe (a public URL by its ORIGIN ROOT only, or a built-in metric with a closed selector), plus optional `watches/<slug>.json` files (the event-watch pack's own authoring form) so an errand arrives beside the sources it reads. Every surface shows a URL probe by origin, never a URL. Refused by name at publish, install and the registry: `at` one-shots, condition and event whens, verb schedules, every digest kind, WRUN metric ids, `dependencies`, `stream` / `share` / `door_policy` blocks, `history/` files, more than 32 members (routines and watches combined), a member over 64 KiB. A pack carries no channel and no workspace: it declares `needs` the installing home answers (a screenshot: channel + workspace descriptions, 160 chars each; an errand: channel only; a watch: none). An errand member also never ships a model or an authority: the installing home's default model runs it, and the installer seals its own arm.

- **Publish** (§"Publishing and yanking" for consent): `package_publish` with the source `schedule:<id>` (CLI: `om publish schedule:<id>`) exports one local schedule as a member: `channelName` becomes `needs.channel`, a screenshot's `payload.workspaceId` becomes `needs.workspace`, an errand's model and authority are stripped, and any other resource refuses with its field named. `with_watches` (CLI: repeatable `--with-watch <slug-or-id>`; the schedule ref form only) carries LOCAL event watches into the pack; a watch the user follows but does not own refuses `not_owned`, and a group label never ships. The result's `notes` field lists what was stripped or mapped by FIELD NAME, never a local value. While the registry predates the kind the publish validates locally and uploads nothing; say "the registry does not accept routine packs yet" and hand the user the pack directory for a local install.
- **Inspect**: `package_show_routine` (CLI: `om package show <source>`, no flag) returns the kind-aware member roster (kind, name, trigger kind, cadence, probe origin or metric selector, an errand's fenced prompt excerpt, a watch's source adapter, the needs) joined with what this home has installed of it (`installed`: schedule id, paused or armed or stood down, the last-fire summary from the reliability ledger) and `registry.accepts`; its `next_steps` name the one default move. It never returns a probe URL, probe args, a raw spec or a full errand prompt (that renders only at a terminal: `om package show-routine <source> --member <name> --full-prompt`).
- **Install**: `package_install` from a local directory or tarball (`@scope/name` only once the registry learns the kind). Preflight resolves ONE channel through the routing ladder (a card-only chat refuses) and, only when a screenshot member needs one, ONE verified workspace, computes every member's cadence and probe origin, the watch plan (create paused, or update in place: journal, history and enabled state kept) plus the replace roster (one fleet per package name: a re-install retires the earlier fleet, and the card lists those retirements, including a dropped watch's destructive removal), and binds the card to a digest over that exact preview. The card renders the preview; never supply the digest yourself, and a `re_preview` outcome (`preview_mismatch`, `roster_changed`) means show the fresh preview and ask again. Members land PAUSED, provenance stamped in the schedules state-store sidecar, never on the spec file.
- **Arm**: per kind. A screenshot member: `schedule_resume` raises a card naming the channel, the workspace, the cadence and the probe origin (never just the id); one yes arms one member, the card is bound to that generation's spec hash, and a spec edited between card and dispatch refuses. An errand member: chat and MCP refuse the arm; hand the user the terminal two-step, `om schedule adopt <id>` (the confirm shows the pack's provenance and the complete prompt; `--yes` and non-TTY refuse; on a VPS, SSH with a PTY) which seals it paused, then `om schedule resume <id>` which enables it. A carried watch: `om event-watch resume <slug>`. Local schedules resume card-free as before (`schedules.md §"Routine packs"`). Verify with `schedule_show` and relay the next fire.
- **Fork**: CLI only, no lane verb. `om fork <source> --member <name> [--workspace <id>]` creates ONE member as an owned, paused schedule with `forked_from` in the sidecar lineage and no package directory; `package_install` never forks, and a pack carrying errand or watch members refuses the fork today (screenshot-member packs only). Say so and hand the user the command.
...
- `not_a_routine_pack`: `package_show_routine` was aimed at another kind; `details.kind` names it (a strategy template shows through `package_show_strategy`).
- `member_invalid` / `envelope_refused`: a routine-pack member or its manifest failed the SDK gate (an `at`, condition or event when, a WRUN metric id, a URL with a path or query, a `stream` / `share` / `dependencies` block, a `history/` file, the member or count caps); the message names the file and the rule. Fix the pack; nothing was created.
...
- `watch_collision` / `watch_source_changed`: a pack watch slug or id is already taken by a watch this package does not own (or a skipped spec that cannot be identified), or an owned incumbent under the slug watches a DIFFERENT source than the pack ships. Nothing was written. Remove or rename the local watch, or (source change) re-publish the member under a new slug; never delete the user's watch yourself.
- `package_arm_requires_terminal`: `om schedule adopt` of a package-installed errand was called with `--yes` or without a TTY. Arming one requires a present human reading the full prompt: hand the user `om schedule adopt <id>` for an interactive terminal (VPS: `ssh -t <host> om schedule adopt <id>`), then `om schedule resume <id>` enables it.
- `fork_member_kind_unsupported`: `om fork` was aimed at a pack carrying errand or watch members; forking supports workspace_screenshot members only today. Install the pack instead.
...
- `registry_inert`: a routine-pack publish or registry install while the registry predates the kind. Say "the registry does not accept routine packs yet" and install from a local directory or tarball.
...
- `package_install`
  - `workspace` — routine-pack installs: the chart workspace id every routine renders (defaults to the active chart workspace).
- `package_publish`
  - `scope` — Defaults to the registry_scope setting.
  - `version` — Group form only: semantic version for the generated pack (MAJOR.MINOR.PATCH; defaults to 0.1.0 on a first publish).
  - `approved_assembly` — The approval surfaces write this after the human saw the plan; omit it to publish the group as it stands.
- `wrun_source_set`
...
- `om install` (action: `package_install`) — Install an OpenMarket package from a local path, GitHub, or the registry: WRUN indicator packages (wrun-indicator), kScript indicator sources (kscript-indicator), alert recipe packs (alert-recipe-pack), ontology packs (ontology-pack), strategy templates (strategy-template), event-watch packs (event-watch-pack), and routine packs (routine-pack: schedules created PAUSED; the card shows the exact preview and supplies `approved_preview_sha256`; a `consent_missing`, `preview_mismatch` or `roster_changed` refusal means nothing installed: show the fresh preview it carries and ask again).
...
- `om package show` (action: `package_show_routine`) — Inspect a routine pack before or after installing: validate the pack and return its kind-aware member roster (workspace_screenshot and errand schedules plus carried event watches: name, trigger kind, cadence, probe ORIGIN or metric selector, an errand's prompt digest excerpt, a watch's source adapter, the needs) joined with what this home has installed of it (schedule id, paused or armed, last fire summary from the reliability ledger) and the registry's stance on the kind.
- `om package show-routine` (action: `package_show_routine`) — Inspect a routine pack before or after installing: validate the pack and return its kind-aware member roster (workspace_screenshot and errand schedules plus carried event watches: name, trigger kind, cadence, probe ORIGIN or metric selector, an errand's prompt digest excerpt, a watch's source adapter, the needs) joined with what this home has installed of it (schedule id, paused or armed, last fire summary from the reliability ledger) and the registry's stance on the kind.

- `om publish` (action: `package_publish`) — Validate a package directory and publish it to the OpenMarket registry (first publish under an unclaimed scope claims it), or publish a composite by `group`.
