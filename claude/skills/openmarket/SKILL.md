...
- **WRUN packages**: `om install` installs WRUN packages from local paths, GitHub repos, or registry tarballs; `om wrun ...` scaffolds, validates, builds, exports, installs, lists, removes, upgrades, and configures WRUN indicator packages. Installed outputs become metric ids like `wrun/@scope/name/output` that work with metric lookup, alerts, and chart adds (`om chart indicator add --type wrun/...`). See `marketplace.md` for the registry workflow and agent-native authoring.
...
- **Charts**: `om chart ...` to inspect and mutate OpenMarket chart workspaces through a persistent WebSocket the daemon holds open to `collab-service`. Verbs sit directly under `om chart`, with `indicator` and `drawing` as grouped families. Reads: `om chart status`, `om chart list`, `om chart refresh --workspace <id>`. Mutations: `om chart layout` (multi-chart grid — `3x1`/`2x2`/...), `om chart symbol` / `om chart interval` / `om chart plot-type` (per-pane), `om chart sync` (symbol/interval/crosshair sync), `om chart indicator add` (RSI/MACD/EMA/...), `om chart drawing add` (TrendLine, FibonacciRetracement), `om chart view` (set the visible time range — ephemeral). See `chart-actions.md` for the full workflow. A grid/layout request is an `om chart layout` change FIRST, then per-pane `om chart symbol`.
...
- `marketplace.md`: discover, install, and publish registry packages (`om search` / `om install` / `om publish` / `om yank` / `om access`), plus agent-native WRUN indicator authoring (`wrun_author` → `wrun_build` → value-check → publish) with the metadata dialect and styling vocabulary. Read when the user mentions the marketplace or registry, wants to install or publish a package, or asks for a custom indicator to be built, styled, or put on a chart.
- `metrics.md` — compute scalar metric values ad-hoc via `om metric get`, scan a universe via `om metric screen`, and discover the registry via `om metric list`. Covers every registered metric (price / delta_pct / volume / funding_rate / open_interest plus indicators: RSI / SMA / EMA / MACD / BB / ATR / Stoch) with canonical default params and prompting rules; CCI/MFI/OBV/VWAP/ADX/Ichimoku are chart overlays only, not metrics. Read when the user asks for the current value of any scalar metric on a symbol ("what's RSI on BTC?", "what's the price of SOL?", "give me the 4h MACD for ETH", "is BTC overbought right now?", "show me Bollinger Bands on SOL") or wants to verify what an alert would fire on right now.
- `chart-actions.md` — the chart surface: agent commands for inspecting the orchestrator's WS connection (`om chart status`), listing workspaces (`om chart list`), reading the live workspace state (`om chart refresh --workspace <id>`), and mutating charts (`om chart layout`, `om chart symbol`, `om chart interval`, `om chart plot-type`, `om chart sync`, `om chart indicator add`, `om chart drawing add`, `om chart view`). Read when the user asks "is the orchestrator/bridge connected?", "what workspaces do I have?", "what's on my chart workspace?", or asks to: **see / show anything on a chart** ("show me X", "put it on the chart", price action, a level, a comparison) / change the **layout / grid** (e.g. `3x1`, `1x3`, `2x2`, "three across", "split into two") / change a chart's symbol / interval / plot type / toggle multi-chart sync (symbol / interval / crosshair) / add a technical indicator (RSI / MACD / EMA / BB / ATR / Stoch / Liquidations) / seek / zoom / pan the visible range. For **drawing** tools (trendlines, fibs, shapes, positions, annotations) see `chart-actions-tool-drawing.md`. **A grid dimension or "set up N charts with these symbols" is a `chart layout` change FIRST, then per-pane `chart symbol` — read this file before acting.**
- `chart-actions-tool-drawing.md` — the drawing-tool detail for `om chart drawing` (auto / add / schema / remove). Read when the user asks to draw, add, place, mark, pin, box, annotate, highlight a level / zone / trendline, plot a fib, set up a long/short position, or remove a tool on a chart (any "mark it / pin it / draw it on the chart" phrasing). Covers `drawing auto` (anchors computed from market data — preferred), `drawing add` (caller-supplied role-tagged `--anchor`s), `drawing schema` (discover a tool's anchor roles), and `drawing remove`.
...
- `research.md` — correlational event studies over accepted event-watch rows and one asset's candles. Read when the user asks whether a watched event stream coincided with market moves, wants to inspect event anchors, or wants pre/post returns around events.
...
- `rooms-docs.md` — the living-docs layer of OM Rooms chat: the space library (one head per doc, CAS revisions, om://doc pills) and the `doc_*` lane verbs (list/read/search/history/write plus move/revert/archive/restore/access). Read when working inside a chat room with files or docs: the user asks what files are in this channel, wants a conversation captured into a doc, references an om://doc pill, or asks to edit, rename, file, revert, share, archive, or restrict a doc.
...
```ts
// packages/cli/src/skills/package.ts
import skill from "../../../../skills/SKILL.md" with { type: "text" };
import alerts from "../../../../skills/alerts.md" with { type: "text" };
import orders from "../../../../skills/orders.md" with { type: "text" };
import metrics from "../../../../skills/metrics.md" with { type: "text" };
import marketplace from "../../../../skills/marketplace.md" with { type: "text" };
import chartActions from "../../../../skills/chart-actions.md" with { type: "text" };
import eventWatches from "../../../../skills/event-watches.md" with { type: "text" };
import news from "../../../../skills/news.md" with { type: "text" };
import research from "../../../../skills/research.md" with { type: "text" };
import signal from "../../../../skills/signal.md" with { type: "text" };
import strategy from "../../../../skills/strategy.md" with { type: "text" };
```
