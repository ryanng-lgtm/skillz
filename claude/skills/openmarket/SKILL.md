...
- `watch.md`: ONE noun for everything watched and everything that runs on it. Sources (§"Sources"): a market condition (§"Condition source"), a page (§"Page source"), a feed, an X handle, a hosted search, a timer (§"Timer source"), an upstream watch (§"Upstream source"), an inbound door; actions (§"Actions"): screenshot, errand, verb, execute (§"Execute action", §"Strategy execute") and judge (§"Judge action"); arming (§"Arming"), reads (§"Read a watch"), edits (§"Edit a watch"), lifecycle (§"Lifecycle"), budgets (§"Budgets and caps") and the share / follow / install / publish / fork / mute / knock doors (§"Doors"). Read when the user wants to be told when a market crosses a level, a page changes, a subject posts or a cadence comes due; wants work or an order to run on that moment; asks what they are watching or why a watch is quiet; or wants to share, follow, install or publish one; also what this home follows and any knock waiting or denied (§"Doors").
...
- "Keep me posted on <subject>" / "watch <subject> for me" / "tell me whenever <subject> posts about <topic>" naming a SUBJECT (a person, organization, product, or public issue; the topic rides in `intent`) → `watch_compose` (`om watch`): one verb researches sources, probes them, creates the grouped watch set with notify on, and returns the judged current read (sources' own items, first-party verified) to open the reply with. A published news Topic already covering the subject is followed AS WELL (`watch_follow`), into the same group: both, for now, the Topic's curated coverage plus the composed first-party legs, and some duplicate pings on a hot story are the accepted cost (the notification cooldown is the remedy, news.md §"Noise and duplicates"). A Fast alert (news.md §"Author a Fast alert") remains the moment-shaped ask, and a single named feed/handle/metric keeps its own verb. A CADENCE brief ("every weekday at 8, tell me only if X said something new") is NOT a subject watch: it is a timer watch with one errand carrying `web_research` (watch.md §"Timer source"), no composer.
- "Watch this URL for new items" (a page, document or JSON endpoint read by a model on change) → `watch_page_add` (`om watch <url>`), never `watch_create`; a change clause ("when it changes do X") rides the SAME call as an `actions[]` errand, one card for both. "Every morning do X", "when watch Z lands a row run Y" → `watch_create` with a `timer` or `upstream` source plus `actions` (watch.md §"Timer source", watch.md §"Upstream source"); an errand, verb or execute action lands disabled unless the one enabled one is armed by the card, and "arm it" is `watch_action_arm` (watch.md §"Arming"). "What is it doing / is it armed" → `watching_overview`.
...
- An automation ask routes by what its condition DESCRIBES, not its phrasing. A REGIME — a state to hold a position through ("a bot", "trade this rule for me", when to BE long/short/flat; an indicator threshold reads as one) → one watch with a `judge` action and a `strategy` execute reading it (watch.md §"Judge action", watch.md §"Strategy execute"), created paper with derived defaults. A TRIGGER — a moment to act on when the condition fires → a condition watch with an `order_on_fire` execute action (watch.md §"Execute action"), `max_fires: 1` by default; repeated one-sided ADDS with no exit rule ("buy $N more each time …") → the same shape with raised caps. Truly unable to tell → ONE structured question (the paper strategy beside the conditional order); asking it on a clear regime ask is a miss. Whichever shape an automation ask lands on, the reply names the other reading in one line.
- "Alert me when BTC crosses 100k", "when funding goes negative", "when RSI drops under 30" → `watch_create` with a `condition` source (watch.md §"Condition source"); "the moment" / "as soon as" → `latency_class: "fast"`.
