...
- `marketplace.md`: discover, evaluate, install, and publish registry packages (`om search` / `om install` / `om publish` / `om yank` / `om access`; strategy templates ride the try-before-install arc — `package_try` evaluates a tuned candidate and mints the install token), and the WRUN marketplace — mount an installed indicator on a chart, bind an odds input to a market, repoint a pinned one, list, remove, and routine packs (schedule-only, installed paused; `marketplace.md §"Routine packs"`). Read when the user mentions the marketplace or registry, wants to install or publish a package, or wants an installed WRUN indicator on a chart or bound to a market; building one is `wrun.md`.
...
- A question about a news event, a catalyst, or what someone said or posted ("did X happen?", "what did they say?", "what did @handle post?") → the journal first, in this turn without asking (`event_journal_search`, passing the window the user named; `event_watch_events`; event-watches.md §"Read journals"), plus the odds read (`polymarket_odds`, last-traded) when the subject trades. The search result's `live_fetch` says what comes next: `run` (nothing answered the asked window) means `web_research` in the same turn for a named subject, then say what the journal lacked; `offer` means answer from the journal and END with one line naming the live fetch as available (it is the one read offered rather than run: cents, and a walled conversation). Words that name the live world (search, look up, live, a pasted URL) go to `web_research` first (`page_read` for the URL itself). A new watch covers the future, not this turn; nothing captured, nothing to research, no market: say so in one line and offer a watch once.
...
- "Keep me posted on <subject>" / "watch <subject> for me" / "tell me whenever <subject> posts about <topic>" naming a SUBJECT (a person, organization, product, or public issue; the topic rides in `intent`) → `watch_compose` (`om watch`): one verb researches sources, probes them, creates the grouped watch set with notify on, and returns the judged current read (sources' own items, first-party verified) to open the reply with. A published news Topic already covering the subject is followed AS WELL (`news_follow`), into the same group: both, for now, the Topic's curated coverage plus the composed first-party legs, and some duplicate pings on a hot story are the accepted cost (the notification cooldown is the remedy, news.md §"Noise and duplicates"). A Fast alert (news.md §"Author a Fast alert") remains the moment-shaped ask, and a single named feed/handle/metric keeps its own verb. A CADENCE brief ("every weekday at 8, tell me only if X said something new") is NOT a watch: it is one `schedule_create` errand with `web_research` (schedules.md), no composer.
...
- An automation ask routes by what its condition DESCRIBES, not its phrasing. A REGIME — a state to hold a position through ("a bot", "trade this rule for me", when to BE long/short/flat; an indicator threshold reads as one) → a signal/strategy pair (strategy.md §"When to use this skill vs `openmarket-alerts`"), created immediately with derived defaults, never an execute alert or hand-rolled script alert. Truly unable to tell → ONE structured question (derived strategy plan beside the conditional order); asking it on a clear regime ask is a miss.
- A TRIGGER — a moment to act on when the condition fires → an alert with an `on_fire.execute` block (alerts.md), once by default; repeated one-sided ADDS with no exit rule ("buy $N more each time …") → the same block with explicit `fire_mode: "recurring"` and raised caps. Whichever surface an automation ask lands on, the reply names the other reading in one line.
