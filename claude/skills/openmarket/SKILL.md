...
- **Indicators**: `om indicator ...` (the same verbs as `om wrun ...`) scaffolds, validates, builds, exports, installs, lists, removes, upgrades, and configures Indicator packages (registry kind `wrun-indicator`); `om install` installs published ones from the registry, local paths or GitHub repos. Installed outputs become metric ids like `wrun/@scope/name/output` that work with metric lookup, alerts, and chart adds (`om chart indicator add --type wrun/...`). kScript (legacy) scripts stay on the chart's own engine. See `marketplace.md` for the registry workflow (install, mount, bind, repoint, list, remove) and `wrun.md` for authoring one.
- **Watches**: `om watch ...` is the one noun for everything a person keeps an eye on and everything that runs when it moves: a market condition, a page, a feed, an X handle, a hosted search, a timer, another watch or an inbound door as the source, with `ai`, `tool` and `money` steps on each row, joined in chains, and delivery as a setting. `om service install` registers the launchd/systemd daemon that evaluates them and delivers to configured channels. See `watch.md`.
...
- **Charts**: `om chart ...` to inspect and mutate OpenMarket chart workspaces: the daemon holds a WebSocket open to `collab-service` for the state snapshot and every edit that follows, and posts each mutation to it as an intent over REST. Verbs sit directly under `om chart`, with `indicator` and `drawing` as grouped families. Reads: `om chart status`, `om chart list`, `om chart refresh --workspace <id>`. Mutations: `om chart create` (a new workspace — REST, no daemon needed), `om chart layout` (multi-chart grid — `3x1`/`2x2`/...), `om chart symbol` / `om chart interval` / `om chart plot-type` (per-pane), `om chart sync` (symbol/interval/crosshair sync), `om chart indicator add` (RSI/MACD/EMA/...), `om chart drawing add` (TrendLine, FibonacciRetracement), `om chart view` (set the visible time range — ephemeral). See `chart-actions.md` for the full workflow. A grid/layout request is an `om chart layout` change FIRST, then per-pane `om chart symbol`.
...
- `watch.md`: ONE noun for everything watched and everything that runs on it. Sources (§"Sources"): a market condition (§"Condition source"), a page (§"Page source"), a feed, an X handle, a hosted search, a timer (§"Timer source"), other watches and one watch with several sources (§"Upstream source"), an inbound door; steps (§"Actions"): ai (§"AI step"), tool (§"Tool step") and money (§"Money step", §"Strategy mode") in chains, delivery as a setting; arming one chain per card (§"Arming"), reads (§"Read a watch"), edits (§"Edit a watch"), lifecycle (§"Lifecycle"), model use (§"Model use"), the share / publish (one watch or a set as one package) / follow / install / fork / mute / knock doors (§"Doors") and the typed refusals (§"Errors"); the build of one watch from a sentence, the section `watch_kit` serves with this home's facts (§"Build from a description"), trigger versus context roles on sources (§"Trigger and context roles") and `@{Name}` mentions (§"Mentions"). Read when the user wants to be told when a market crosses a level, a page changes, a subject posts or a cadence comes due; wants work or money to run on that moment; asks what is watched or armed, or why a watch is quiet; or wants to share, follow, install, publish or fork one (§"Doors").
...
- `marketplace.md`: discover, evaluate, install, and publish registry packages (`om search` / `om install` / `om publish` / `om packages delete` / `om access`; strategy templates ride the try-before-install arc, `backtest_run` evaluates a tuned candidate and mints the install token), the Indicator marketplace (mount an installed Indicator on a chart, bind an odds input to a market, repoint a pinned one, list, remove) and watch packs: one watch or a set of watches as one package with its sources, chains and dependency pins, installed paused, every chain armed on the installing machine (`marketplace.md §"Watch packs"`). Read when the user mentions the marketplace or registry, wants to install or publish a package, asks what is installed or published, wants a package updated, uninstalled or deleted (`marketplace.md §"Installed packages"`), or wants an installed Indicator on a chart or bound to a market; building one is `wrun.md`.
- `wrun.md` — build a custom sandboxed Indicator (a `wrun-indicator` package, the `wrun_*` tools) from a plain description (`wrun_author` → `wrun_build` → preview → publish): the authoring loop, the metadata dialect, input pins, bindable odds inputs, chart styling. Read when the user asks for an indicator to be built, styled, or its metadata written; installing, mounting, binding, repointing or removing an Indicator package stays with `marketplace.md`.
...
- `news.md`: the news store, vendor linking, catalog discovery and previews, subscriptions, billing, briefs and voice. A saved feed opens its watch; authoring, history, editing, delivery and replay use `watch.md`. Chart pins use `chart-actions.md`.
...
- `research.md` — backtests and event studies. Read FIRST for any backtest / "would it have worked" ask (§"Reach for backtest_run FIRST": `backtest_run` on a saved watch or an installed package, sweeps via `backtest_sweep`), for correlational event studies (§"Event studies"), and for reading one named web page or document (§"Page read (the `page_read` tool)": origin card, untrusted text).
...
- Anything to be told about LATER, or to have something RUN later ("tell me when", "alert me", "ping me", "watch <subject> for me", "whenever <subject> posts about <topic>", "keep tabs on", "every morning at 8", "when Y happens do Z", a URL or an X account to watch, a rule to trade, "build me a watch that ...", a change to a watch) → `watch_kit` FIRST, in the turn it was asked (watch.md §"Build from a description"): one read whose desk reads the person's own words, names the ONE door they open and attaches it with its arguments bound, beside the shapes, every recipe and this home's channels, providers, venues and watches. Then ONE draft (a request in words through `watch_compose`, a URL through `watch_page_add`, a pasted handle through `watch_create`; a market rule, a timer, a search or a model question reach their verbs only through the composer's handoff, and a build the request did not open is refused), its `card` shown verbatim, and the arm card in the same turn (a chain with a money step arms at the terminal, watch.md §"Arming"). A feed beside a price rule is context (watch.md §"Trigger and context roles"); an ai step's prompt is written by `watch-prompts.md`. "What is it doing / is it armed" → `watching_overview`.
...
`om setup <name>` pairs one connection per platform, and `/setup` in a terminal `om chat` runs that same guided connect inline — from a chat surface, name `/setup` first and the terminal verb second. Routing is **materialized** — a watch stores exactly its `notify.channel` (a channel id), and there is no read-time fan-out. A create with no `channels[]` is **seeded** to the configured **default channel** (set via `om setup default <name>`), or the lone channel when only one is configured; with several channels and no default set the create is **refused** (pass `--channel <name>` or set a default: `om setup default <name>`); with no channels configured it is card-only (no push, no agent take). Pairing the very first channel also routes any still-card-only watches onto it, so specs authored before a channel existed get a real destination (a later second channel re-homes nothing). Once a Discord, Slack or Telegram connection is paired, its bot reaches every channel it can post in as a destination, addressed as `discord:#ops`, `slack:C0123ABCD` or `telegram:-1001234567890` (`:<thread>` for a thread or topic), or `#ops` when exactly one paired connection reaches a channel of that name. `om channel add <address>` mints a destination without routing anything to it; `om channel home <address>` sets where the agent greets you and daemon wakes land. `om channel <name>` runs `om channel show <name>`, its default subcommand.

| Channel | Setup |
| --- | --- |
| `telegram` | `om setup telegram --token <bot-token> --chat-id <id>` (or interactive) |
| `discord` | `om setup discord` (interactive; bot token) or `--webhook-url <url>` for a one-way single channel |
| `slack` | `om setup slack` (interactive; bot token) or `--webhook-url <url>` for a one-way single channel |
...
Interactive mode (no credential flags) prompts for the channel name first (defaulting to the adapter id, e.g. `telegram` / `discord`) before collecting credentials — same as `om init`. Pass `--name <name>` to pair a second connection on the same platform (a second bot, or a webhook) instead of the default row. Pass `--default` to mark the new channel as the default for watches without an explicit destination.
...
From the daemon itself only a crash, an unclean start, a blind X handle and a dead credential reach a chat; a planned start or stop, a room roundup, a short catch-up gap and a stalled loop are recorded in `om notis` instead. The default-channel preference applies to watch fires.
...
Whenever the runner is up (foreground `om run` or background `om service install`), it also exposes a local HTTP surface on `:31337`: `/healthz`, `/rpc/v1/watch/*` (watch workers and runtime control), `/rpc/v1/money/*` (money-step views), `/rpc/v1/money-digest/*` (daily money-step digest), `/events/v1` (SSE stream: `watch_fired` with `kind: "condition" | "listener"`, `channel_delivered`, `channel_failed`, `tick_completed`), `/api/v1/*` (REST proxy to upstream), and `/ws/v1/*` (WebSocket proxy). The CLI uses the daemon routes for operations owned by the running daemon. Watch authoring and history reads also have direct local paths; `/events/v1` is the live event stream, not the stored history API.
