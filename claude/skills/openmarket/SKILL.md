...
- `research.md` — backtests and event studies. Read FIRST for any backtest / "would it have worked" ask (§"Reach for backtest_run FIRST": `backtest_run` on a saved strategy or an unsaved candidate, sweeps via `backtest_sweep`), and for correlational event studies (§"Event studies": whether a watched event stream coincided with market moves, event anchors, pre/post returns).
