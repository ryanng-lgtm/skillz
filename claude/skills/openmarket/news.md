...
- Buying, upgrading or cancelling is never yours — name the exact terminal command (`om news subscribe`, `om news attach <id>`) and never state a price (§"Wall phrasing").
- `active_quota_exhausted` is cleared by freeing a slot the user picks — never answered with a purchase (§"Wall phrasing").
- Six `news_*` calls raise a card in every approval mode (`watch_remove`, `watch_remove`, `watch_unfollow`, `watch_resume`, `watch_share`, `watch_unpublish`), as do five shapes: `news_edit` carrying `alertWhen`, `news_replay` starting or cancelling a sweep, `news_brief` scheduling or sending off-box, `chart_pins` onto a chart the user keeps, and `backtest_news` with `materialize: true` (§"Publish, follow, fork").
- Publishing is a disclosure: the alert's condition becomes world-readable; confirm intent in your own words before `watch_share` (the one publishing verb, live by default; `live: false` publishes the recipe alone), ahead of the card it raises (§"Publish, follow, fork").
...
- "Stop pinging me about X" is a mute (`watch_mute`), never a delete — the feed and its journal keep running (§"Noise and duplicates").
...
Auto-attached watches default to `llm_every_event`; `accept_all` is an explicit opt-out; a backfill walks pages with a resume cursor and spends no budget.

Every auto-attached watch, Attention or Synoptic, defaults to `llm_every_event`: the local classifier checks each incoming item against the watch's goal, keeps what matches, and drops the rest; every item is judged, there is no daily budget (watch.md §"Model use": the judge model and the user's key are the one control). Explicit `om watch backfill` walks the history one page at a time (`--limit` is the page size) and its reply carries the resume cursor and the counts; a run the deadline or the page stops says so and resumes from `--from <resume_cursor>`. Goal quality is the product: a vague goal spends reads on noise, a sharp one keeps the journal clean.

`accept_all` — journal and notify every delivered item with no local classifier — is an explicit opt-out, not a default. Only switch a watch to it on the user's ask (`om watch edit <id> --classifier-mode accept_all`, or `om config set news.auto_watch.classifier.<vendor> accept_all` for future auto-watches).
...
Before creating, check `om news list --format json` and `om watch list --format json`: the user may already own, follow, or be covered for the topic. Prefer editing an existing Fast alert, or forking a published one, over creating a near-duplicate.
...
On an interactive terminal, `om news attach <id>` for a paid Stream offers the purchase: pick a billing option, a hosted checkout page opens in the browser, and om waits for the Stream to unlock and then adds it as usual. Non-interactive and `--format json` runs never prompt — they return `purchase_required`.
...
- **A per-target Stream whose envelope names no usable step size is refused before anything is posted.** With no step size there is no step count to send for any target count, so om buys nothing and points at synoptic.com; after buying there, `om news attach <id>` adds it normally.
...
A Stream sold per target delivers nothing until targets are set on it — `om watch follow <target> --feed <id>`. An add of one says so; the wording is conditional because whether a list is already in place is not readable per user.
...
Publishing is a disclosure and cards; the carded `news_*` set and its exceptions live here — follow is read-only, `watch_fork` copies to an owned alert, a declined card answers.

- **Publishing is a disclosure.** It makes the Fast alert's `alert_when`, `query`, and tracked entities world-readable in the catalog. Confirm intent in your own words before calling `watch_share` (the one publishing verb: live by default, `live: false` publishes the recipe alone), and expect the surface to confirm too: on chat surfaces a publish raises an approval card naming what goes public. **Six `news_*` calls raise a card**: `watch_share` when it publishes, on every vendor, because who may list a feed is answered per account and the card follows the direction of the call (a Topic publish is refused for most callers with `briefs_topic_authoring_restricted`, and discloses the Topic for one of Attention's curators; with `room` the same verb authorizes an open-ended stream of cards into a room under the user's name), `watch_unpublish` (it takes one published version back down with the author's note: holders keep their copies and nobody is notified), `watch_remove` (permanent, and it retires the feed's event-watch), `watch_remove` (it unfollows the stream vendor-side — gated by name, so every removal cards, including the free ones), `watch_unfollow` (it retires that feed's watch the same way: the follow is re-added from the catalog at any time, the watch's stored events, backbone, snapshots and room shares are not; two shapes retire nothing: a Synoptic target drop (an unfollow carrying `feed`, which leaves the feed and its watch standing), and a feed with no watch of its own to retire), and `watch_resume` (it re-opens a standing delivery and claims one of the account's active slots). Five more shapes card while the rest of their verb does not: `news_edit` when it carries `alertWhen` (a trigger rewrite permanently discards the history a replay swept for the sentence it replaces, and no later run brings those rows back — §"Replay a Fast alert's history"), `news_replay` when it starts a sweep (a `lookback`) or cancels one (the progress poll starts and stops nothing, so it reads free — §"Replay a Fast alert's history"), `news_brief` with `mode: "schedule_set"` or a `generate` naming a channel (each stands or sends a briefing off-box — §"The daily brief"), `chart_pins` when it targets a chart the user keeps — an explicit `workspace`, or `here: true` for the one they are looking at (either re-points that chart and leaves pins on it that only `chart_delete` removes; the agent's own minted charts and the default day workspace dispatch without a card — §"Plotting events on charts"), and `backtest_news` when it carries `materialize: true` (the paid timeline synthesis runs outside the run's own call cap and persists reconstructed snapshots — §"Backtest before arming anything"). Every other `news_*` call follows the one approvals mode (it dispatches in auto and takes a one-line ticket in ask): a `news_edit` that only renames a feed or retunes a Topic's `interest` (neither touches what a sweep wrote), `watch_pause`, and `watch_unshare` included. Three more cards this skill raises from outside the family, all on the watch side: `watch_remove`, the legacy-watch cleanup (§"After any acquisition, verify the pair"; permanent destruction of paid output, so it cards like the removals do), and `watch_resume` and `watch_edit`, which re-open and re-point a watch's delivery. No approval mode and no session allowance answers any of these — the card appears in every mode — so make the disclosure it will name in your own words before calling. A declined card is the user's answer: do not re-ask or route around it.
- **Several feeds are ONE call, one approval.** Every lifecycle verb here (`watch_remove`, `watch_remove`, `watch_unfollow`, `watch_follow`, `watch_pause`, `watch_resume`, `watch_mute`, `watch_unmute`, `watch_share`, `watch_unshare`) takes `ids` beside its single selector, with one `provider` per call (and one `room` for the share pair); one approval covers the set, and where the verb raises a card that card lists every member with its fate (which watch a removal retires, which it keeps; the slot a resume claims; the share state a room binding has); ids the vendor does not know are skipped rows and never dispatched, and a member that fails mid-batch never voids the others. The vendor round-trip verbs (delete, remove, unfollow, follow, pause, resume) cap at 50 per call and stop at the first rate limit with the rest reported `failed: rate_limited` (never retried; re-run the rest later); the local verbs (mute, unmute, share, unshare) cap at 200. Never loop single-id calls for a set: on the verbs that card that is one card per feed, and on the rest one dispatch per feed. The CLI mirrors it: `om watch delete|pause|resume <id...>`, `om watch remove|mute|unmute <feed...>`, `om watch follow|unfollow <target...>`, `om watch share|unshare <feed...> --room <room>`.
- **Follow is read-only.** The user receives fires but cannot edit the condition. To customize a Fast alert, `om watch fork <id>` copies it into an authored one they own (which holds a slot). The canonical flow: `catalog`, then `show <id>` to read the condition, then `fork`, then `edit`, then optionally `publish`. A Topic has no such flow: the catalog is curated, so following IS the way to hold one, and the interest stays as its publisher wrote it.
...
- When the user creates a news watch *because of* a market conversation (an alert they just made, a chart they are discussing), tag it: `watch_create` with `related_markets`, or `om watch edit <id> --market BINANCE:BTCUSDT` for an existing watch (repeat `--market` per ref; `--clear-markets` removes them). Auto-attached news watches are not tagged automatically, because only the conversation knows which market motivated them.
...
`watch_share` posts fire cards into a room AS THE USER — the instruction must name the room, confirm both ends, `room_not_joined` means join first; `watch_unshare` reports counts.

`om watch share <feed> --room <room>` (action `watch_share`) binds a feed to an OM Chat room: every fire that passes the feed's own notification gates then posts a compact card into the room, at most one per 10 minutes (major updates always post). Doctrine:
...
- A muted or suppressed fire never reaches a room: shares ride the same gates as the user's private delivery, so quieting a feed quiets its shares too. Say so when a user asks why a muted feed stopped posting. `watch_share`'s own result carries `delivery`, `daemon_running` and `vendor_paused` — read them before telling the user cards are on their way.
- A `degraded` share (see `watch_show` / `om watch show`) stopped posting after repeated failures; re-running `watch_share` for the same feed and room re-arms it. A `blocker` on a row is the other silence: nobody is signed in, the feed is muted, its watch is paused or gone, or another account on this install owns the binding, and each has its own fix — re-arming clears none of them.
- **`watch_unshare` removes every binding of that feed to that room**, and `removed_count` says how many. Re-sharing afterwards starts a new share with zeroed delivered/suppressed counters, so report the count rather than "stopped the share".
...
- `muted: true` means the feed runs and nothing pings; matches keep landing in the journal and the `/news` panel. `watch_unmute` reverses it.
...
For what a watch has seen, use `watch_history` (`om watch events <id>`) or `om event-journal get <slug>` (rolling overview). `watch_history` takes the watch id as OPTIONAL: omit it for a merged, source-time-ordered page across every watch — the honest "what fired recently, in order" read — and pass one for a single watch's page. Either way the page is bounded (50 rows by default, 500 at most): counts come from `total_matching`, never from how many rows arrived, and `next_cursor` goes back as `before` for the next page. watch.md §"Read a watch" carries the rest of the paging contract. Pull only the journal relevant to the question; do not bulk-load journals into context. Correlation against price action belongs to `research.md`; wiring fires to automated trading belongs to `strategy.md`.
...
3. `watch_pause` that id (or `watch_unfollow` a follow they no longer want), then retry the original `news_create` / `watch_fork` / `watch_follow` / `watch_resume`.
...
- `backtest_news` · `news_attach` · `news_catalog` · `news_create` · `news_edit` · `news_list` · `news_package_streams` · `news_packages` · `news_preview` · `news_replay` · `news_show` · `news_voice`
