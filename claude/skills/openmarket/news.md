...
- Six `news_*` calls raise a card in every approval mode — `news_delete`, `news_remove`, `news_unfollow`, `news_resume`, `news_share`, `news_publish` unless `public: false` — as do four shapes: `news_edit` carrying `alertWhen`, `news_replay` starting or cancelling a sweep, `news_brief` scheduling or sending off-box, and `chart_pins` onto a chart the user keeps (§"Publish, follow, fork").
...
- "stop pinging me about X": a **mute** (`news_mute`, reversed by `news_unmute`), never a delete. The feed and its journal keep running; only the pings stop. Several feeds = ONE `news_mute` / `news_unmute` call with `ids` (one card lists every member; one receipt block in auto mode).
...
- `backtest_news`
  ...
  - `max_llm_calls` — Default: 500 — a classify run always carries a ceiling, and reaching it refuses with the spend so far, every verdict already paid for durably cached.
...
- `om news pause` (action: `news_pause`) — Pause an owned Fast alert (stops it firing without deleting it): `id` for one, or `ids` for several in ONE call (one approval card covers the set; a member already paused is reported unchanged).
