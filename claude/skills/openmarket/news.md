...
- Five `news_*` calls raise a card in every approval mode (`watch_remove`, `watch_remove`, `watch_unfollow`, `watch_resume`, `watch_share`), as do five shapes: `news_edit` carrying `alertWhen`, `news_replay` starting or cancelling a sweep, `news_brief` scheduling or sending off-box, `chart_pins` onto a chart the user keeps, and `backtest_news` with `materialize: true` (§"Publish, follow, fork").
...
- `news_attach`
  - discloses `eventWatch.notify_style` — The created watch's own pick: normal (a sentence or two) or full (the card). Absent: what the channel reads
