...
`om news` is the control plane for text-event feeds: news, social posts, government releases, anything that arrives as text rather than a number on a chart. Three vendors sit behind one capability-gated command set:
...
**Routing:** name a moment and the user wants a Fast alert ("tell me WHEN X happens" — latency matters), which is authored; name a subject and they want a Topic ("keep me posted ON X" — coverage matters), which is followed from the curated catalog; want somebody else's ready-made feed and they want a stream. Name a place the user owns or names ("watch my IBKR emails", "watch our CI", "watch this URL") and the request leaves the vendor lanes entirely: the information lives at their place, not in the world's coverage, so read `connect-source.md` and build the inbound pair there instead. Topics are never called alerts/triggers/fires in user-facing copy, never described with urgency language, and never advertised as screened or verified.
...
- The user wants an alert on a news/social/text event ("tell me when X happens", "watch for Fable 5 news").
- The user asks what news alerts or feeds they have, or wants to change/quiet/remove one.
- The user wants to browse or follow other people's published Fast alerts, or fork one to customize it.
- The user complains about duplicate or too-frequent news pings.

## One-off answers convert once (offer doctrine)

"What's happening with X?" answered from news or web data is a one-off read; a watch is the standing version of the same question. The conversion is offered, never pushed:
...
- **A decline is remembered.** When the user declines the offer (or waves it off), save that with the memory tools (memory_save, after the memory_search-first rule in the persona's memory doctrine) keyed to the subject, and never re-offer that subject unprompted, this session or later ones. A remembered decline is lifted only by the user's own ask.
...
Every auto-attached watch — Attention or Synoptic — defaults to `llm_every_event`: the local classifier checks each incoming item against the watch's goal, keeps what matches, and drops the rest, under per-watch daily LIVE budgets (default 1000 classified items, 2000 LLM calls per UTC day — leave both alone unless the user asks; see the event-watches skill for the confirm-back rule on lowering one). Explicit `om event-watch backfill` never spends these live budgets; it runs on its own per-run LLM-call ceiling and, when that ceiling binds, returns `status: budget_paused`, a choice to relay (continue from the resume command, raise the ceiling, or stop), never a completed backfill (see the event-watches skill). Goal quality is the product: a vague goal wastes budget on noise, a sharp one keeps the journal clean.
...
- There is no `news_setup` action: vendor linking beyond Attention's lazy link is interactive-only. When Synoptic linking is missing, tell the user to run `om news setup synoptic` in a terminal.
...
## Author a Fast alert: always preview first
...
One field authors a Fast alert: `--alert-when`, the moment that matters, in plain English. Write it as "Fire only when …": one kind of event, the specific companies, people or sources involved, and then what should NOT fire ("Do not fire on rumors, speculation…"). The closing exclusion is what separates an alert from a news feed — without it the condition names a subject, and everything that mentions the subject becomes a notification. `--query` — the subject matter to watch — is OPTIONAL on both `preview` and `create`, and leaving it out means something different on each. `create` hands the trigger over and Attention derives the subject for the stored alert. `preview` has no stored alert to derive from, so the trigger sentence stands in as the subject for that one back-test. Retrieval keys off the subject, so a preview run with no `--query` shows the SHAPE of the alert — roughly how often it fires, and on what kind of story — not the exact fires the created alert will produce; say it that way when you report the count. Never invent a subject. Pass one only when the user names it themselves, and then pass the same one to `preview` and to `create` — a subject named at preview and dropped at create (or the reverse) previews a different alert from the one you then arm.

`--window-hours` accepts 1..168. Report the count the way it comes back: two marks, two different claims about a number, and neither is a tally. `approx` (rendered with a `~`) means the count is an ESTIMATE — the vendor sampled rather than exhausted what it counted, so the real figure could be either side of it, and a "likely more" verb means the condition is broader than the number suggests. `atLeast` (rendered as a trailing `+`, "`50+ times it fired`") means the count is a FLOOR — every one of them happened and there were more we did not read, so it can only be under; say "at least 50", never "about 50", which would tell the user their alert might have fired FEWER times than the fires you are showing them. NEVER present either as exact, and never explain how the vendor arrived at it. Calibrate with the user: too many fires means tighten `--alert-when` (it respects explicit "DO NOT fire on ..." clauses); zero fires on a visibly live topic means loosen it or add `--tracked-entities` (comma-separated names that always count as a match). A `hint` in the result is that judgement already made for you — a trigger that would fire more than ten times a day is a feed, not an alert — so relay it and offer to narrow the trigger before you create anything. Then create with the agreed condition:

```bash
om news create \
  --alert-when "..." \
  --tracked-entities "Binance,Coinbase" \
  --label "New perp listings" --format json
```

Always name what you author: `--label "<short name>"` on the CLI, `title` on the actions (the interactive console asks for it in its own dialog). The name is what every surface shows and what the feed's watch is addressed by; without one a Fast alert is named after the subject it watches (the one passed, else the one derived from the trigger), clipped to 60 characters — not a name a person would have chosen.

Edit later with `om news edit <id>` (any subset of the condition fields, `--label` to rename); pause/resume/delete manage lifecycle. Changes are live against the vendor immediately.
...
3. **Tighten the condition** (Attention): add "DO NOT fire on ..." clauses via `om news edit`, or narrow `--query` so retrieval reads a smaller slice.
...
- "stop pinging me about X": a **mute** (`news_mute`, reversed by `news_unmute`), never a delete. The feed and its journal keep running; only the pings stop.
...
- "kill the feed" / "delete it": actual removal, which cards (see the publish section). Confirm it is not a mute in disguise before destroying anything.

## Publish, follow, fork

- **Publishing is a disclosure.** It makes the Fast alert's `alert_when`, `query`, and tracked entities world-readable in the catalog. Confirm intent in your own words before calling `news_publish`, and expect the surface to confirm too: on chat surfaces a publish (`public` true, which is also the default when the flag is omitted) raises an approval card naming what goes public. **Five `news_*` calls raise a card**, and they are the whole set in that family: `news_publish` when it resolves public — on every vendor, because who may list a feed is answered per account and the card follows the direction of the call (a Topic publish is refused for most callers with `briefs_topic_authoring_restricted`, and discloses the Topic for one of Attention's curators) — `news_delete` (permanent, and it retires the feed's event-watch), `news_remove` (it unfollows the stream vendor-side — gated by name, so every removal cards, including the free ones), `news_share` (one call authorizes an open-ended stream of cards into a room under the user's name), and `news_chart` when it carries an explicit `workspace` (it re-points a chart the user keeps and leaves pins on it that nothing removes). Every other `news_*` call dispatches straight away — taking a feed back down (`public: false`, `om news unpublish <id>`), `news_unshare`, and a `news_chart` on the feed's own workspace included. The one card this skill raises from outside the family is `event_watch_remove`, the legacy-watch cleanup above: permanent destruction of paid synthesis output, so it cards like the removals do. A declined card is the user's answer: do not re-ask or route around it.
...
## Pair watches with markets (catalyst next to the move)
...
`om backtest news <feed>` (action `backtest_news`) answers "would trading this feed's fires have made money?" with zero authoring: a free correlational study first (does price move after fires?), then a P&L replay of a synthesized hold-after-fire strategy. Run it before proposing any signal or strategy on a feed's fires; the printed breadcrumb at the end names the exact `om signal create` + `om strategy create` commands. The study lane and `--side` replays cost no LLM calls; `--classify` spends real ones (durably cached, so reruns are free) and classifies each fire with the signal's default prior context, rebuilt as of that fire (`--context-replay normal|advanced` selects how; refused with `--study-only`). Relay the result's `disclosures` lines verbatim, especially the context-mode and sparse-fires warnings. The asset defaults from the watch's single `related_markets` tag: tag first, backtest second.

## Fires on the chart

`om news chart <feed>` (action `news_chart`) pins a feed's fire history onto a live price chart: one annotation per accepted fire at its source time ("[major] <title>"), on the feed's OWN clean workspace ("OM News · <label>", reused on re-runs; re-plots only add new fires). The result carries the live-view URL: share it, and relay every `disclosures` line. Doctrine:

- **Chart on request.** When the user asks to "see the fires on the chart", "show me where it fired", or wants news next to price action, call `news_chart`. Do not hand-build this from `event_watch_events` + drawing tools: the verb owns selection, framing, caps, and replot dedup.
- **Asset comes from the tag.** The chart needs a market; a feed tagged with exactly one `related_markets` entry charts with no flags. Untagged (or multi-tagged) feeds need `asset`, and tagging once (`om event-watch edit <id> --market EXCHANGE:SYMBOL`) makes every later chart and backtest flag-free. Tag first, chart second.
- **Offer once, not naggingly.** When the conversation is about a market that one of the user's tagged feeds covers (discussing its price alert, its backtest, its chart), offer the pin chart ONE time ("want the fires on the chart next to it?"), and never re-offer after a decline.
- Topics do not fire, so `attention-briefs` feeds are refused with `feed_not_chartable`; chart a Fast alert or Stream instead.
- Only pass `workspace` when the user names an existing chart to draw on: the run re-points that chart and leaves pins on it that no verb removes, so the call raises an approval card and the CLI confirms before the same projection. The default (the feed's own workspace) is always safe and needs no confirmation.
- Pins select like the backtest replays: fires observed live by default, `history: backfill` for vendor-pulled history, optional `min_confidence` floor and `outcomes` subset (e.g. majors only). A run pins at most the newest 500 fires onto the chart's event lane, or the newest 60 as drawn annotations where the stack has no pin support; the disclosures say when a window was cut.
- **A selection is its window first.** The window is the last 30 days unless `from`/`until` say otherwise, and `from: "0"` charts the whole journal (the result's `window` then reports the coverage it clamped to, and `om news chart --all` is the same reach from the CLI). Every result states the `window` it charted beside the `fires` it matched: relay both, and read an empty selection as its window — offer the wider one before calling a feed quiet.
- **A young journal never claims a month of silence.** Recording starts when the feed's watch is created and flows forward only: a feed added two hours ago has two hours of local history no matter what window was asked. When charting or backtesting a feed whose recording is younger than the asked window (the chart result carries `watch_recording_since` and flags it `young_watch`; both verbs disclose it when the selection is empty or sparse), say that recording just began and relay the recording-age disclosure, then offer the backfill pair ONCE: `om event-watch backfill --watch <id> --from <date>`, then re-run with `history: backfill`. Never present an empty young journal as "no news happened".
- **Follow keeps the chart live; unfollow stops it.** Charting the feed's own workspace binds it by default: the daemon pins each NEW fire on as it happens, with the run's filters frozen in. Explicit `workspace` targets follow only when `follow: true` is passed. `unfollow: true` removes the binding (existing pins stay); the result's `follow` block is the status surface, and a `degraded` binding (repeated push failures) re-arms with `rearm: true`, which resets its health in place. Never re-arm by re-running the plain verb: that re-projects, and a re-projection can repoint the binding at another workspace or widen the filters frozen into it. Relay that state when the user asks why a chart went quiet.

## The daily brief: "what happened today?" / "catch me up"

`om news brief` (action `news_brief`) rolls everything the user's feeds produced into ONE synthesized, deduplicated briefing: stories merged across feeds, major developments first, a quiet note for feeds with nothing new, nothing repeated from the previous brief unless it advanced. In `om chat` the `/brief` slash renders the same edition as a transcript block. When the user asks "what happened today", "catch me up", "anything new on my feeds?":
...
- If it is stale or missing (`not_found`), call mode `generate` (one LLM call when the user has one configured; the fallback is an honest raw fire list and says so via `generator: "fallback"`, which you should mention).
...
- `generate` spends an LLM call and `schedule_set`/`schedule_off` mutate durable state; prefer `get_last` whenever freshness allows.
...
- **Posting the daily brief into a room is not a share** and needs no new tool: fetch it with `news_brief` mode `get_last` and post `body_md` with `room_message_send`, only when the user asks for that room by name.

## Answering "what am I watching?"
...
- `slots` inside `free_beta` is **the beta balance**: what the window grants while it is open, with `slots_free` already counting against it. It is ABSENT once `active` is false: never substitute the top-level `quota` (that is what the account bought) and never state a number for the beta balance. Holding every slot in it answers `active_quota_exhausted` — Wall 1 below, cleared the same way: free one, retry. Never answer a full beta balance by suggesting a purchase.
- Say following curated feeds is free with or without the beta, every time the end date comes up.

`free_beta` ABSENT means there is no such window: do not mention one, and do not infer it from anything else. It is also absent when `exempt` is true — that account holds alerts without limit, so never quote it a beta balance. Never state a price for the beta: the agent surface carries no prices at all.
...
## Injected news context (the `<news_context>` fence)
...
- Depth is ONE tool call: `event_journal_get` with the slug the line carries (or `news_overview`), only when the question needs more than the injected rows hold. Do not fan out over journals from an injected line.
- No fence means the message matched nothing tracked: answer normally with the tools above.
...
- Catch-up shape, fixed: one opener line with the counts, then per thread ONE short connective sentence plus its story citations, then a quiet line and one tail hint (`/news`, or `/brief` for the daily edition). Nothing else.
- Question shape, fixed: a 2-3 sentence answer in plain prose, then the cited receipts, then at most one offer line.
...
In `om chat`, fires render inline as cards, the header's news slot carries the latest headline (or `N new` while fires are unread), and an arrival greeting lists what landed while the user was away. Four commands:

- `/news` — the news home, and the only news surface: three lanes (`FAST`, `TOPICS`, `SYNOPTIC: STREAMS`) over an ON WATCH card that lists the most recently active feeds with their delivery and age, plus a dim plan line under the masthead. `↑`/`↓` rows, `←`/`→` lanes, Enter opens the row (a feed's detail, a catalog row to follow or add), `n` takes the focused lane's action row (create a Fast alert, browse Topics, browse the Synoptic marketplace), `f` jumps to the fires panel, `r` refreshes, `$` opens the billing pane, Esc returns to chat. Enter on ON WATCH opens the watchlist: every feed the user owns or follows — Inbound included — with its delivery state, tabbed All/Fast/Topics/Streams, `m` mutes a row in place, `r` re-reads the page, `q`/Esc goes back to the home. Every screen is the same console `om news list` opens, so a follow, an add, a create or a delivery change behaves identically and keeps its confirmations.
...
- `/news side` — the docked rail of recent fires. On by default; it needs 110+ columns (140+ with the member panel open), and the choice persists. The bare form toggles, so `/news side on` and `/news side off` are the forms to name when the user says which way they want it — each says so rather than flipping whatever state it found.
...
When a user asks where their news went, point at `/news`; when they ask to see recent fires, `/news fires`. `/news` is the single news entry point — there is no `/news browse`, so never offer it. On non-TUI surfaces answer the same questions directly from `event_watch_events` and the journals.
...
- `om news chart` (action: `news_chart`) — put a feed's fire history ON the price chart.
- `om news create` (action: `news_create`) — Author and create a Fast alert from alertWhen (Attention); `query` is optional and derived from alertWhen when omitted.
- `om news delete` (action: `news_delete`) — Permanently delete an owned Fast alert (Attention) or Topic (provider 'attention-briefs').
- `om news edit` (action: `news_edit`) — Edit an owned Fast alert's fire condition, subject, or tracked entities.
...
- `om news mute` (action: `news_mute`) — Stop a feed's pings WITHOUT stopping the feed.
...
- `om news pause` (action: `news_pause`) — Pause an owned Fast alert (stops it firing without deleting it).
...
- `om news remove` (action: `news_remove`) — Remove one locally managed feed (Synoptic).
- `om news resume` (action: `news_resume`) — Resume a paused Fast alert (Attention) or a paused Topic (provider 'attention-briefs').
- `om news setup` — (bespoke; see narrative above)
- `om news share` (action: `news_share`) — share a feed's fires into an OM Chat room.
- `om news shares` (action: `news_shares`) — list the user's room shares: which feed posts into which room, delivered/suppressed counts, and health (`at_risk` is failing but still trying; `degraded` stopped posting after repeated failures, re-arm it with news_share).
...
- `om news unfollow` (action: `news_unfollow`) — Attention: unfollow a public Fast alert by id; unfollowing another publisher's frees the slot it held.
- `om news unmute` (action: `news_unmute`) — Resume a muted feed's pings; the result's `delivery` says where they land (channels, or inline in om chat only when none is configured).
...
- `om news unshare` (action: `news_unshare`) — stop sharing a feed's fires into a room.
