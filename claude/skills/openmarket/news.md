...
Every auto-attached watch — Attention or Synoptic — defaults to `llm_every_event`: the local classifier checks each incoming item against the watch's goal, keeps what matches, and drops the rest, under per-watch daily LIVE budgets (default 1000 classified items, 2000 LLM calls per UTC day — leave both alone unless the user asks; watch.md §"Budgets and caps" carries the confirm-back rule on lowering one). Explicit `om watch backfill` never spends these live budgets; it runs on its own per-run LLM-call ceiling and, when that ceiling binds, returns `status: budget_paused`, a choice to relay (continue from the resume command, raise the ceiling, or stop), never a completed backfill (watch.md §"Lifecycle"). Goal quality is the product: a vague goal wastes budget on noise, a sharp one keeps the journal clean.
