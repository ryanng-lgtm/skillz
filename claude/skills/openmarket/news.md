...
- Six `news_*` calls raise an approval card — `news_publish` (unless `public: false`), `news_delete`, `news_remove`, `news_unfollow`, `news_share`, and `news_edit` when it carries `alertWhen` — plus `chart_pins` when it targets a chart the user keeps. Expect the card, and treat a declined card as the user's answer (§"Publish, follow, fork").
