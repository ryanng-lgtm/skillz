...
- `on_fire.execute` IS the user's authorization to submit an order when the alert fires: include it only for a TRIGGER — a moment to act on — never for a notification ask; a REGIME ask (a state to hold a position through — "a bot", "trade this rule for me") is the signal/strategy pair (`skill_read("strategy")`), not an alert. One complete action defaults to `fire_mode: "once"` — one ping, one execution; repeated one-sided adds with no rule-driven exit stay HERE, with explicit `fire_mode: "recurring"` and raised caps; whichever surface the ask lands on, the reply names the other reading in one line (§"Auto-execution").
- An execute block with no `brackets.stop_loss_px` gets the question "Add a stop-loss?" via the structured-question tool (recommend yes); never add one silently, and ask it only once the ask is genuinely an alert — never to settle which surface it is on.
...
- A composite is confirmed ONCE under its group name: one row per leg (its label, what it watches, a metric leg's reading in one clause from its own create result) and one delivery row (§"Grouping legs under one named watch").
...
- A ONE-SHOT "Do X when Y" is an alert carrying `on_fire.execute`, and so are repeated one-sided adds (explicit `fire_mode: "recurring"`, raised caps); "do X now" is an order — `skill_read("orders")`; a REGIME — a rule saying when to BE in a position, managed over time → the signal/strategy pair (`skill_read("strategy")`) — decided by intent, never by JSON shape, and never a synthetic always-true alert.
...
One line saying what is armed, in the user's words with every default named; the readings as one clause per leg and the `routing_note` as one row; venues and metrics are words, never registry ids. No closing offer.
...
- **Manage**: `alert_hosted_list`, `alert_hosted_history`, `alert_hosted_pause` / `alert_hosted_resume`, `alert_hosted_remove` (destructive: confirm first). Several hosted alerts at once are ONE call with `ids` (up to 25 per call; `om alert hosted pause|resume|rm <id...>`): never loop single-id calls for a set. Pause and resume follow the writes mode — one ticket in ask mode, one receipt block in auto mode; the remove raises one ticket in every mode, listing every member with its status and rule. A platform rate limit stops the remainder as `failed: rate_limited`, never retried; re-run the rest later.
...
**Classify an automation ask by what its condition describes — a REGIME or a TRIGGER — before authoring anything here.** A REGIME is a state to hold a position through: "a bot", "trades … automatically", any rule saying when to BE long/short/flat — the textbook read of an indicator threshold ("long while RSI is oversold"). That is the signal/strategy pair — stop and `skill_read("strategy")`; that skill derives sizing and stops from safe defaults, and the stance itself exits and re-enters each episode, so recurring phrasing alone never keeps an ask on this surface. A standing strategy in observe or paper mode is WALLETLESS — it needs no paired venue and no `om setup`; the pairing requirement above gates only the wallet side (this surface's execute block, or a strategy's dry_run/live modes — the modes that touch a real wallet). Never bounce the user to pairing as a prerequisite for the strategy path: on a clear regime ask, read the strategy skill and create immediately with derived defaults, without bouncing to setup prerequisites first (the strategy skill's own few question-worthy cases — real money, an unresolvable market — still stand). A TRIGGER is a moment to act on when the condition fires, and stays here: one complete action is an execute alert, `once` by default; repeated one-sided ADDS with no rule-driven exit ("buy $100 more each time …" — a stance strategy cannot buy again while already long) are also this surface's, as an execute alert with explicit `fire_mode: "recurring"` and deliberately raised `caps`. Genuinely unable to tell regime from trigger is the TIE: ask ONE structured question and nothing else. Option A, recommended: the derived standing strategy — state the plan inline (walletless observe or paper; sizing and stops derive automatically, so say so, never ask for them). Option B: the conditional order — an execute alert that places the order when the condition fires. The question is one message carrying exactly those two options; no size, stop, venue, or pairing sub-question rides along with it. Until the answer arrives: no other asks and no setup steps — no size question, no stop-loss question, no venue or pairing errands, no arming language (nothing is created yet). Then act on it. Reaching for this question on a clear regime ask is itself a miss — classify first. The stop-loss question is asked only once the ask is settled as genuinely an alert — never as a way to resolve which surface the ask is on. Whichever surface an automation ask lands on, the reply names the other reading in one line ("built as a standing paper strategy — say the word for a one-time conditional order instead", or the mirror), so a misread costs one user turn.
...
5. **Preview only where no approval card exists.** On a surface with an approval card the card is the preview, and a plain `alert_create` dispatches with no preview at all (the guardrail above): the outcome line names its defaults. Where no card exists (Telegram, Discord, Slack) and the ask needs one, the preview is labelled rows, one field per row, every default marked `(default)`, never prose.
...
6. **Confirm with the structured-question tool (external agents only; the om agent's plain create dispatches, per step 5).** Options: `Yes, create it` / `Edit a field` / `Cancel`. On `Edit a field`, ask a follow-up structured question with the editable fields as options (fire mode, expiration, threshold, market), then re-prompt for the new value and loop back to step 5 with the updated preview.

7. **Persist.** On `Yes`, call `alert_create` with the spec object as its arguments (the top-level fields — `label`, `condition`, `fire_mode`, `cooldown`, `expires_at`, `channels`, `group` — are the call's own arguments; there is no wrapper key). `alert_import` is the door for a spec the user hands you complete and wants stored as written; `alert_create` is the authoring door, and the one that reads the market (§"Polymarket alerts"). Report success in one line with the returned `id` and what is armed, then the `routing_note` as one row; never restate the field summary the user just approved. Example: *"Created alert 3: BTC > 95k recurring, expires 2026-05-20."*
...
- **Path C — everything**: *"pause everything"* → **always** confirm first via structured question (`Yes, pause all N alerts` / `Cancel`). On confirm, call `alert_pause` ONCE with `ids: [...]` (one ticket in ask mode, one receipt block in auto mode; a shell user has `om alert pause <id...>` or `--every-alert`). Report the count. `alert_resume` with `ids` refuses the whole call (`batch_arming_member`) on either of two counts, each named separately in the error: a member that would ARM (a script alert, or one carrying an execute block) — resume those one at a time so each arming card stands alone — and a member this build cannot READ, which no batch can prove toothless: that spec is repaired or removed first, and naming an arming alert instead sends the user after the wrong file.
...
- **Do not second-guess the user's threshold.** If they say "more than 95k," produce `op: gt, value: 95000` — transcribe the level literally, and never suggest a different value. Direction is the one thing you may question: when the stated value sits far from the current reading (or the condition is already true as written), surface the current value and confirm the direction in one structured question before arming, as the Polymarket create flow's sanity-check step does.
...
- `alert_create`
  - discloses `routing_note` — Where the alert's fires post, in one sentence, with the command that moves it. · Where this set's fires post, as one `Delivery:` row with the command that moves them; on a channel-less home it names `om setup` and the edit that routes one.
...
- `om alert hosted pause` (action: `alert_hosted_pause`) — Pause hosted (chart indicator) alerts: `id` for one, or `ids` for several in ONE call (one ticket in ask mode, one receipt block in auto; never a loop of single calls).
- `om alert hosted resume` (action: `alert_hosted_resume`) — Re-arm paused hosted (chart indicator) alerts on the platform engine: `id` for one, or `ids` for several in ONE call (one ticket in ask mode, one receipt block in auto).
- `om alert hosted rm` (action: `alert_hosted_remove`) — Delete hosted (chart indicator) alerts from the platform engine: `id` for one, or `ids` for several in ONE call (one ticket covers the set, in every mode; never a loop of single calls).
