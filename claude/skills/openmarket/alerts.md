...
Top-level fields and defaults — fire_mode, cooldown (60s, quiet not blind), expires_at, latency_class, channels — and how the user's verb picks an edge or level op.
...
- User doesn't mention rate-limiting at all → omit the field. The runner applies the 60s default, which is almost always what the user wants — it neutralizes spam on level-op recurring alerts (e.g. `price gt 80000`) without blocking legitimate edge-op fires (edge state already holds those to one per crossing).
...
Everything not sent arms on its default — `fire_mode` recurring, `cooldown` 60s, no expiry, the conversation's channel — and the result's `readings[]` carry where each leg stands right now, which is the arming sentence's material.
...
When debugging a stuck or misbehaving script alert, peek at its memory with `alert_state_show` `{ "id": "<id>" }`: it returns the JSON the daemon last persisted as `next_state`, plus a byte count and a warning if it's above 75% of the 1 MiB cap.
