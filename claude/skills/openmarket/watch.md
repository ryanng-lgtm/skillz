...
- "Pause / resume / delete watch 3" → `watch_pause` / `watch_resume` / `watch_remove` by id; a watch with sources sweeps every source; several at once = ONE call with `ids`; delete cards (§"Lifecycle").
...
| "publish the desk as one package" | `watch_share` with `group` (or `refs` + `name`) | one install-only pack: the sources, the chains, the pins; `with_execute` ships money steps' terms (never box values) |
...
5. **Preview before enable.** `watch_test` with `preview: true` (`om watch test <ref> --preview`) runs the chain on the watch's last accepted row (a condition watch: its sample fire) with every delivery and money step stubbed, and returns the would-be card: the row, each step's would-run line, where it would have been delivered (§"Preview before enable"). Cover three cases before arming: the matching event (the preview: which steps run, what each would do, where it goes), a rejected event (`watch_history` with `outcomes: ["irrelevant"]` shows what the filter dropped and why, so the user sees the filter working), and a missing context (`watch_show` on each context source: one with no rows yet says so, and the step runs without it). Testing arms nothing and sends nothing.
6. **One readable card, the activation controls, then the state.** Say the watch in the product's words, one sentence per role: "BTC crossings trigger it. Your macro feed supplies context. The model evaluates the update and sends its explanation to Telegram." Then the required activation controls: the arm card per chain (`watch_arm` from chat for a chain with no money step; a money chain returns `om watch arm <watch> <chain>` to run at the terminal: §"Arming"), and `watch_resume` when the watch is paused (an installed or imported one). Call `watch_arm` in the same turn: the card is how the user says yes; describing the arm in text and ending the turn leaves the watch off with nothing to answer. Verify with `watch_show`: the state you report is the one it prints (`armed at chat`, `armed at terminal`, `off`, `declined at <step>`), never the one you expect.
...
One watch with several sources: `watch_create` with `sources[]` (each `condition:<json>`, `every:<cadence>`, `at:<instant>`, `upstream:<watch>`, `stream-ref:<json>` or `inbound`; the CLI spelling is `--source <kind>:<value>`, repeatable) creates one source watch per entry under one group and one upstream watch over all of them that carries the steps, and prints one card. From then on every surface shows ONE watch: `om watch list` prints `<label> · N sources · M chains · <state>`, `om watch show` lists each source with its own last fire and what its rows arrive as, then the chains; `watch_pause`, `watch_resume`, `watch_remove` and `watch_share` on the watch sweep every source, and `--source <slug>` (the CLI) or the source's own id narrows to one. A page, search or model source goes through its own door first (`watch_page_add`, `watch_search_add`, `watch_model_add`), then rides as an `upstream` entry.
...
`watch_test` with `preview: true` (`om watch test <ref> --preview`) runs the watch's chain on its last accepted row (a condition watch: its sample fire) with every delivery and money step stubbed: nothing sent, placed, spawned or recorded, and nothing armed; any watch kind. The result's `preview` is the would-be card: `row` (the journal row it ran on: `event_id`, `title`, `summary`, `outcome`, `received_via`, `created_at`, and `sample: true` for a synthetic fire), `steps` (one per step: `id`, `kind`, `enabled`, and `line`, what the step would do on this row, `(disabled: would not run)` while its chain is off) and `deliver` (one line per destination the fire would have reached). A watch that has not fired yet answers `not_fired_reason` (an inbound watch's names the `om event push` that gives it a row). Show the card in the product's words, one sentence per step. Without `preview`, `watch_test` is the sample fire of a condition watch to its own channels: a real send, never bulk. Preview, then arm, then `watch_show`; never arm a chain nobody has seen run.
...
- `watch_share`
  ...
  - `which_fires` — `every` (default) forwards every accepted row
