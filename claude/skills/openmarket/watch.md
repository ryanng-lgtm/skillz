...
| "ask the model every hour whether ..." | `watch_model_add` | one card: the model and whose credential, the prompt whole, the tools in the menu's words with the calls, where it reaches out, the answer shape, the ceiling with your daily cap; the member saves at once and the daemon's first read lands as the opening items (`--probe-now` for the sample at once); never on MCP |
...
2. ONE `watch_create` carrying everything: `label`, `source` (one) or `sources[]` (several, each with `name` and `role`), `notify`, `actions[]`; pass `enabled: false` when steps ride it. A URL, a hosted search, a model question or a subject goes through its own door first (`watch_page_add`, `watch_search_add`, `watch_model_add`, `watch_compose`, each with `enabled: false` where it takes one) and rides the create as `{kind: "upstream", watch_ids: [<its id>]}` inside `sources[]`. One draft, never two.
...
5. No steps? The watch runs from the create (`enabled` defaults to true): show the card and say in one line what it watches and where it posts. Nothing to preview; `watch_test` with `preview: true` is for chains.
...
| a web page, a document, a JSON endpoint | `watch_page_add {url, intent?, cadence_sec?, enabled: false}` first | every 15 minutes; only items matching `intent` when one is given, else every change; then `upstream` |
| an RSS, Atom or JSON feed | `{kind: "listener", name, stream_ref: {adapter: "feed", channel: "items", extra: {url}}, user_goal, exclude?}` | every item when `user_goal` is broad; the full arm: `schema_read {intent: "event_watch_create"}` |
| an X account | `{kind: "listener", name, stream_ref: {adapter: "x", channel: "posts", extra: {handle}}, user_goal, exclude?}` | polled every 5 minutes on the home's X lane (the kit's `home.x` says which) |
| a person, company, product or public issue ("watch <subject> for me", "keep tabs on") | `watch_compose` | it researches and probes the subject's real channels, creates the grouped set with notify on and opens the reply with a dated read; pass `sources` only for a handle or URL the user named |
| a standing question a model answers on a cadence, keeping what it finds as rows | `watch_model_add {group: "<the watch's label>", name, prompt, every, tools?, output: "rows" or "answer", enabled: false}` first | hourly; `tools` absent = every read plus web search; then `upstream` |
| a search the model runs on a schedule | `watch_search_add {group: "<the watch's label>", subject, intent, search: {engines: ["web"], cadence_sec}, enabled: false}` first | web, daily, 2 searches a run; paid |
| the user's own script, CI, inbox, webhook or log pipe | `{kind: "inbound", name, user_goal, daily_cap?, values_rule?}` | the create returns the door and the mint command; `skill_read("connect-source")` |
| another watch, or a followed signal | `{kind: "upstream", watch_ids: [...], roles?}` | the kit's `home.watches` names them; a news feed beside a price rule is `role: "context"` (never a second trigger; §"Trigger and context roles") |
| a scenario or theme ("alert me if <situation> escalates") | a composite: the news leg plus the markets pricing the question as odds legs | at least two entries, the top rows of ONE volume-ranked Polymarket `markets` page, one `group` label; never ask which markets |
...
- Sends: `notify: {enabled: true, channel: "<name>"}` on the watch is where the original update and every result go (`style: "agent"` = the om agent writes the message, `"alert"` = the plain alert; say which); a step's own `channels: [...]` overrides it and `[]` keeps that result in history only. "Only when the decision is X" is the reader's prompt answering `NO_REPLY` on the other words.
...
`instruction` (`user_goal`, `include`, `exclude`, `extra_guidance`, `timezone`) and `classifier` (`mode: accept_all | llm_every_event`, budgets) ride inside a listener or inbound source; on `watch_create` the listener and inbound arms take `user_goal` (required), `include`, `exclude` and `classifier` directly, and the create folds them under `instruction`. On the agent lane the condition, listener and inbound arms are advertised as stubs (the kind, the required fields, one example): `schema_read {intent: "alert_create"}`, `{intent: "event_watch_create"}` or `{intent: "inbound_create"}` loads the full arm into `watch_create` before the call; the timer and upstream arms are complete as shown. Every kind carries the same envelope: `label`, `slug`, `enabled`, `group`, `related_markets`, `notify`, `actions`. The engine keeps one source per watch; `sources[]` on `watch_create` presents several as one watch (§"Upstream source"). Every stored watch carries `spec_version: 2`; a file without it is `unreadable` (§"Lifecycle").
...
Pause, resume, remove, test, repair, the token verbs and the unreadable row; read this before stopping, restoring or deleting a watch.
...
- `watch_replay` (`om watch replay <watch> [<row>|latest]`): "resend that alert", "run the steps on that one again". One stored update through the watch's live path again: sent to the watch's channels once more (the body opens with `(replayed: first seen <date>)`) and its steps and readers run on it. The row is taken as stored (nothing re-fetched or re-judged), the journal is untouched, history gains one `replay` row pointing at the original; `row` is a history row id, an event id or `latest`, and `watch` may be the row's watch or a reader over it. `steps_only` / `send_only` do one half. Refused for a rejected row (`only an accepted update can be replayed`) and a paused watch (`arm it first`). Cards: it delivers for real.
...
- `watch_backfill`: replays a vendor feed's history into the journal under the classifier budget; `watch_lane_retry` (`om watch lane retry <watch>`) re-arms a stopped relay lane.
...
- `roster_warnings` (a result field on `watch_create`, `watch_edit`, `watch_import`, `watch_action_add|remove|rename|adopt` and `watch_test`'s preview, never a refusal): other watches on this home whose own references dangle, each `{watch, code, message}` in the SDK's sentence (`watch 'grouped-pair' reads upstream watch 'grouped-pair-timer', which does not exist`; `action 'w3' of watch 'w' reads 'z/gone', which does not exist`). This call's watch was admitted on its own paths; nothing here needs answering now. Relay them once: `om watch remove <watch>` clears a reader nobody wants, or add its source again. Absent when the home is whole.
...
- `watch_model_add`
  ...
  - `probe` — default "background" — background (default): the member saves at once and the daemon's first run stores its opening items; the row shows it.
...
- `watch_replay`
  - `row` — Which stored update: a history row id (the number `watch_history` shows), an event id, or `latest` (the default: the newest accepted update across the watch's trigger sources).
- `watch_search_add`
...
- `watch_replay`
  - discloses `delivery.status` — `sent` reached the named channels; `skipped` means steps_only; the rest are the notifier's own gates (the watch's notify block, its confidence floor, its cadence), with the reason.
  - discloses `steps` — The enabled steps this replay obligates: the watch's own, then every reader's that takes this watch as a trigger. Empty with send_only.
- `watch_resume`
  - discloses `status` — The watch's status after this call, the sentence every surface prints: `working`, `paused`, or `<part> <state> · <why>` (for example `steps waiting for your OK · you changed them`).
- `watch_retry`
...
- `om watch list` (action: `watch_list`) — List configured watches with their status.
...
- `om watch mute` (action: `watch_mute`) — Stop this watch's channel deliveries: fires keep committing to the journal, nothing is sent until watch_unmute, or until the deadline `for` (a duration) or `until` (an instant) names, after which deliveries resume by themselves.
- `om watch page-add` (action: `watch_page_add`) — : Watch a URL (a page, a document, a JSON endpoint) for new items, as it is.
- `om watch pause` (action: `watch_pause`) — Disable one watch by id or slug, several by exact id in ONE call (`ids`), or every watch in a composite (`group: <label>`); its steps stop with it and get their OK again when it resumes, the money steps on OTHER watches reading its rows are NOT auto-paused, and journals are preserved.
...
- `om watch replay` (action: `watch_replay`) — Run one stored update through its watch's live path again, as if it had just been accepted: it is sent to the watch's channels once more (the body opens with `(replayed: first seen <date>)`) and its steps and readers run on it.
...
- `om watch source add` (action: `watch_source_add`) — Add ONE source to a saved watch.
- `om watch source remove` (action: `watch_source_remove`) — Drop ONE source from a watch that reads several, by the name `om watch show` lists it under.
