...
| "ask the model every hour whether ..." | `watch_model_add` | one card: the model and whose credential, the prompt whole, the tools in the menu's words with the calls, where it reaches out, the answer shape, the ceiling with your daily cap; the probe's findings land as the opening items; never on MCP |
...
1. **Understand the request, then discover what exists.** Say the route back in one sentence (what triggers, what is context, what runs, where it goes). Read before writing: `watch_show` on every watch the user names (its sources by name and role, its chains, its arm state, who owns it), `watch_tools` (the menu: every row a model run may call, when you want it, what comes back, the default set), `markets` and `metric_list` (the market, the metric, an installed indicator's `wrun/...` id), `package_search` (a pack that already covers the ask beats a hand-rolled one).
...
- **The probe and the reply.** After the yes: the seal, the staged watch, ONE run under the sealed terms (it counts as the day's first under the cap), its findings stored as the member's opening items (never notified), then the watch is enabled. A finding may carry an `artifact_id` (an image the run made); the delivery attaches it. The reply is one outcome line: the model, the cadence, the group, how many findings the probe stored, a sample title or two. `status: adopted` means the group already carries this exact run (same prompt, cadence, tools and answer shape) with a standing seal, and nothing new was created; `not_created` carries the reason (the probe refused or failed): the seal was revoked and the staged watch removed, never a half-made, paused watch. A probe that asked for a row the run cannot use says so (`the probe asked for make_image; this run cannot use it; tick it on the menu or change the prompt`).
