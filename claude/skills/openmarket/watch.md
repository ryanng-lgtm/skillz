...
- "Watch <subject> for me", "keep me posted on X", "tell me whenever <subject> posts about <topic>" naming a person, organization, product or issue (the topic rides in `intent`) → `watch_compose` (`om watch <sentence>`): researched sources, probed, one watch per subject (several land in one folder), the judged read to open the reply with (§"Watch a subject").
...
- "Pause / resume / delete watch 3" → `watch_pause` / `watch_resume` / `watch_remove` by id. Pause/resume with one id affects that watch; `ids` is an exact set of watch ids, `group` names a folder (every watch under that label). Delete cards: §"Lifecycle".
...
- Share or publish a watch or a set of watches ("publish my watch", "send this watch to followers", "broadcast this", "let people follow this", "publish the desk as one package", "sell it", "share it into #desk"), "stop broadcasting" (`watch_unshare`), follow, install, fork, mute, knocks → §"Doors".
...
| --- | --- | --- |
| "tell me when ...", "watch ... for me", "build me a watch that ..." (any watch ask, from chat) | `watch_kit` first, then ONE `watch_create` | the brief, this home's channels, providers, venues and watches, the build verbs attached; the draft's `card` shown verbatim; the arm card in the same turn |
...
| "keep me posted on <subject>" | `watch_compose` | one watch per subject with notify on (several in one folder); open the reply with the dated read |
| "every weekday at 8 summarize funding" | `watch_create`, `timer` + ai step | one chain, one yes; the step runs on the default set (every read plus web search) unless `tools` narrows it |
| "ask the model every hour whether ..." | `watch_model_add` | one card: the model and whose credential, the prompt whole, the tools in the menu's words with the calls, where it reaches out, the answer shape, the ceiling with your daily cap; the source saves at once and the daemon's first read lands as the opening items (`--probe-now` for the sample at once); never on MCP |
...
| "stop broadcasting" | `watch_unshare` | closes the stream, keeps the package; no card |
...
2. ONE `watch_create` carrying everything: `label`, `source` (one) or `sources[]` (several; each entry is a source arm plus its `name`, `role` (trigger unless said) and `enabled`; a listener takes `display_name`, an inbound entry is named after the watch), `notify`, `actions[]`; pass `enabled: false` when steps ride it. A URL, a hosted search, a model question or a subject goes through its own door (`watch_page_add`, `watch_search_add`, `watch_model_add`, `watch_compose`, each with `enabled: false` where it takes one): with `watch` naming the watch just created, the door appends its sealed entry to that one file; alone, it creates a one-source watch. One file, never two.
...
| --- | --- | --- |
| a price, an indicator, funding, open interest, volume, a Polymarket probability | `{kind: "condition", name, condition: {metric, selector: {exchange, symbol, interval}, op, value, params?}, cooldown?, fire_mode?}` | "goes / turns / flips / crosses / breaks" = one fire per crossing: `crosses_above` / `crosses_below` (0 for a sign flip; needs `interval`). "is / stays / while above or below" = `gt` / `lt`, re-firing every cooldown while it holds. Several rules: `all` (and) / `any` (or). Polymarket odds on `HOUR` bars, never `latency_class: "fast"`. "the moment / as soon as" on a venue price: `latency_class: "fast"`. Metrics and the tree: §"Condition source"; the full arm: `schema_read {intent: "alert_create"}` |
| a time of day, every weekday, hourly, once on a date | `{kind: "timer", name, cron: "m h * * 1-5", tz}` or `{kind: "timer", at: "<UTC instant>"}` | weekdays at 17:00 in the user's zone when unsaid; a cadence brief about a subject is this shape with `web_research` in the ai step's `tools`, never a subject watch |
...
| an RSS, Atom or JSON feed | `{kind: "listener", stream_ref: {adapter: "feed", channel: "items", extra: {url}}, user_goal, exclude?, display_name?}` | every item when `user_goal` is broad; the full arm: `schema_read {intent: "event_watch_create"}` |
...
**Folders.** A folder is one label (`group`) written on several watches, nothing more: the list shows the folder as one row, `watch_show` with `group` shows every watch in it with its own sources, and `watch_pause` / `watch_resume` / `watch_remove` / `watch_mute` with `group` act on every watch under the label. It gets written for you: a `watch_compose` request naming several subjects folders the watches it writes (the person's own name for the set, else the shortest name in their words), publishing several watches as one package (`watch_share` with `refs` + `name`, or `group`) writes the package label on each, and installing such a package lands its watches in one folder. By hand: `group` (`--group`) on `watch_create` / `watch_compose` / `watch_page_add` / `watch_search_add` / `watch_model_add` puts the new watch in a folder, `watch_edit` with `group` (`--group`) moves a watch, and `watch_edit` with `group: null` (`--clear-group`) takes it out. A one-watch `watch_compose` run with `group` joins that folder and keeps its own label.

**Every other knob, by name** (each described on the attached schema, or on the arm `schema_read` loads; reach for one only when the ask names its behavior): on the watch `group` (the folder: one label over several watches), `timezone`, `related_markets`, `act_within` (act only on fires younger than this); on `notify` `channels` (several), `min_interval_sec` (at most one ping per interval), `min_confidence`, `voice`; on a condition `fire_mode: "once"`, `expires_at`, `goal` (an AI check on the rule's fires: `{user_goal, exclude}`), `latency_class`; on a listener or inbound `include`, `exclude`, `classifier` (`mode: "accept_all"` keeps every item without a model read), `latency_class`; on a model source `max_tool_calls`, `max_runs_per_day`, `recommended_model`; on an ai step `budgetMs`, `maxToolCalls`, `definitions.unsure`; on a search `search.window_days`; on a share `history`, `readme`, `public_outputs` / `which_fires` (what followers receive and when), `member_outputs` / `ship` (a set as one package), `room` (into an OM Chat room instead), `payout`, `with_execute`.
...
`instruction` (`user_goal`, `include`, `exclude`, `extra_guidance`, `timezone`) and `classifier` (`mode: accept_all | llm_every_event`, the judge override) ride inside a listener or inbound source; on `watch_create` the listener and inbound arms take `user_goal` (required), `include`, `exclude` and `classifier` directly, and the create folds them under `instruction`. On the agent lane the condition, listener and inbound arms are advertised as stubs (the kind, the required fields, one example): `schema_read {intent: "alert_create"}`, `{intent: "event_watch_create"}` or `{intent: "inbound_create"}` loads the full arm into `watch_create` before the call; the timer and upstream arms are complete as shown. Every kind carries the same envelope: `label`, `slug`, `enabled`, `group`, `related_markets`, `notify`, `actions`. The engine keeps one source per watch; `sources[]` on `watch_create` presents several as one watch (§"Upstream source"). Every stored watch carries `spec_version: 2`; a file without it is `unreadable` (§"Lifecycle").
...
The arm is a deferred part on `watch_create` and `watch_edit`: `schema_read {intent: "alert_create"}` loads its full schema; once loaded, the condition grammar is written at the `condition` field (the stub carries the example only).
...
The composer: `watch_compose` researches, probes and creates one watch per named subject (several land in one folder), and returns the dated read; read this for "watch <subject> for me".

`watch_compose` takes the user's words as `request`, always (stored on every watch it writes); add the exact names you resolved in `subjects` (PTJ as Paul Tudor Jones) and the intent in `intent` when you separated it. Its judge (one sealed model call) decides the watches: two named people are two watches unless the words relate them (a deal, a dispute, "as one watch"); one name with an intent needs no judge; never group the watches yourself. Pass every name in ONE call, however many (no count limit; one card covers them all); never split a list across calls. Optional `sources` the user named, and the standing searches (web and X ride EVERY compose by default, one paid leg per engine per subject on the lanes this home holds, listed on the card with the exact query, cadence, model and calls; a `search` block only changes engines, cadence or window; `no_search: true` when the user asked for none). It researches where the subject publishes and is covered, probes each candidate deterministically, creates one watch per subject with notify on (a request naming several independent subjects lands them in one folder, named once in the reply; `group` names the folder yourself, and a one-watch run with `group` joins it keeping its own label), and seeds each new watch with a judged sample of its own recent items, returned per source as `read`. The reply is a day-by-day journal of that read, one line per entry with its link, never a count in place of the entries; every compose on a home with a search lane raises ONE card (the feeds, the minted news search and the search legs, with a free-text request parsed into its subjects at card time); a home with no search lane, `no_search`, or a dry run dispatches the plain shape free and leaves its receipt. A published news Topic already covering the subject is followed as well (`watch_follow`) into the same folder.
...
The arm is complete in the `watch_create` schema (the intent `"schedule_create"` needs no `schema_read`).
...
- `watch_retry` (`om watch retry <watch>`): after a fault, clears the engine's stop on the steps, writes back a missing step file and reports the status; it never gives an OK (a step stopped while running unattended waits for `watch_arm`).
...
- `watch_backfill`
  ...
  - `classifier.mode` — default "llm_every_event" — llm_every_event (default): the local classifier scores each item against the goal, keeps matches, drops the rest; every item is judged, there is no daily budget.
...
- `watch_compose`
  ...
  - `group` — Absent: a request the parse splits into several watches lands in a folder named from the person's own words, and a one-watch run stands alone.
...
- `watch_create`
  ...
  - `act_within` — Default: the condition's cooldown (at least 1m), else 5m; a timer has no fence.
- `watch_edit`
  - `source` — On an inbound watch, replace the door's own settings whole (the daily cap, the values rule; an omitted one clears it).
  - `group` — Absent keeps the current folder; null leaves it, and the watches that stay keep theirs (a folder is a label each watch carries, never a record one watch owns).
  - `classifier.mode` — llm_every_event (default): the local classifier scores each item against the goal, keeps matches, drops the rest; every item is judged, there is no daily budget.
  ...
  - `act_within` — null clears it back to the default (the condition's cooldown, at least 1m, else 5m; a timer has no fence).
  - `tools` — `om watch edit <watch> --tools <names|none>`: the functions a model run may call, exactly these ([] = none; absent keeps the current list; watch_tools lists the menu).
  - `aliases` — `om watch edit <leg> --alias <name>` (repeatable): the name variants a search's query carries beside the subject, exactly these ([] = none; absent keeps the stored ones on an unchanged subject).
...
- `watch_kit`
  - `watch` — Omit to build a new watch.
- `watch_knocks`
...
- `watch_model_add`
  ...
  - `max_runs_per_day` — Absent: the cadence alone bounds the day.
...
- `om watch` (action: `watch_compose`) — "Watch <subject> for me" as ONE verb: researches where each subject (a person, organization, product, or public issue) publishes and is covered, verifies sources with deterministic probes, writes one event watch per subject group (independent subjects get a watch each; subjects that only mean something together share one) reading every verified source with notifications enabled, and seeds each with a judged sample of its sources' own recent items, one judge call per source, returned per source as `read`: the reply is a day-by-day journal of it, one line per entry with its link, never a count in place of the entries.
...
- `om watch pause` (action: `watch_pause`) — Disable one watch by id or slug, several by exact id in ONE call (`ids`), or every watch under a group label (`group: <label>`); its steps stop with it and get their OK again when it resumes, the money steps on OTHER watches reading its rows are NOT auto-paused, and journals are preserved.
...
- `om watch source pause` (action: `watch_edit`) — Update a watch's label (its name), goal, filters, extra guidance, classifier, notify, an inbound watch's ingest daily cap, overview, related-market tags, or the brief_condition_fires switch.
...
- `om watch source rename` (action: `watch_edit`) — Update a watch's label (its name), goal, filters, extra guidance, classifier, notify, an inbound watch's ingest daily cap, overview, related-market tags, or the brief_condition_fires switch.
- `om watch source resume` (action: `watch_edit`) — Update a watch's label (its name), goal, filters, extra guidance, classifier, notify, an inbound watch's ingest daily cap, overview, related-market tags, or the brief_condition_fires switch.
- `om watch source role` (action: `watch_edit`) — Update a watch's label (its name), goal, filters, extra guidance, classifier, notify, an inbound watch's ingest daily cap, overview, related-market tags, or the brief_condition_fires switch.
