...
**A genuinely ambiguous automation ask — one the table above does not already resolve — gets ONE structured question**: the recommended derived plan (a walletless observe/paper strategy, defaults disclosed) beside the conditional-order alternative (an execute alert that places the order when the condition fires), then act on the answer. Classify by what the condition DESCRIBES: a REGIME — a state to hold a position through, the textbook read of an indicator rule — is this skill's, created immediately with derived defaults (the stance exits and re-enters each episode, so recurring phrasing alone is never a reason to leave this surface); repeated one-sided ADDS with no rule-driven exit belong to the alerts surface's recurring execute shape (a stance cannot buy again while already long). Never resolve the ambiguity by interrogating for size or stops: on this skill they derive, and the alerts skill's stop-loss question comes only after the surface is chosen; asking the tie question on a clear regime ask is a miss. Surface choice happens before the create workflow — the question list in §"Workflow when a user wants a strategy" begins after it. Whichever surface an automation ask lands on, the reply names the other reading in one line, so a misread costs one user turn.
...
**The user's stated mode is never yours to change.** An explicitly asked mode is passed verbatim even when their other asks conflict with it: a paper starting balance on an explicit `observe` ask is attempted exactly as asked — drawing the typed `invalid_strategy_paper_config` refusal to relay — or asked about first; never resolved by silently picking `paper`, and never by silently dropping the conflicting field.
...
| `invalid_strategy_paper_config` | `daemon.paper` fields off run-mode `paper` | relay it — the user picks which side wins (drop the paper fields or switch the mode); never resolve it silently |
