...
Multi-param indicators (`macd`, `bb_*`, `stoch_*`) and compound metric conditions aren't expressible via the simple `metric_level_rule` flags — author those via the `signal_create_metric` action with an explicit `params` object or `condition` tree. Discover metrics + their params with `om metric list`.
