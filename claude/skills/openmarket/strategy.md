...
**Backtests model no margin; neither backtests nor paper model funding or liquidation.** Leverage scales a result in exposure only, never in liquidation risk — say so on the RISKS line whenever leverage or a perp venue is in play.
...
**A paused strategy is fully hands-off: the daemon places NO orders, so managed exits are NOT enforced while paused** (on live Hyperliquid, already-resting native brackets keep enforcing venue-side; the software time stop and every Polymarket/paper exit stop). Surface the pause output's `held_position` note verbatim and offer the protective alternatives.

⚠️ **Never answer a "one-sided / never short" request with `conviction --on-reversal close_only`.** `close_only` blocks a **one-step flip**, not a two-step reversal: once an opposing view has closed the position to flat, the next fresh opposing view **enters the other side** (worked example: §"Sizer modes and policies"). `single_sided` is the only mode that guarantees the other side is never taken.

**A stance that holds through neutral has no autonomous exit behind a one-sided signal** (`--on-neutral hold`, or `always_in`, on a signal that can never emit the opposing side): nothing ever closes the position — legitimate only as an entry trigger with `--tp`/`--sl`/`--time-stop` attached; `create` warns, heed it.

**Create the signal BEFORE the strategy.** An unresolvable slug stamps the conviction policies from the fallback (for a text signal the exact inverse of its defaults, never re-stamped) and the band rejections stop firing; relay `create`'s warning verbatim. Details: §"Magnitude, capital, and leverage".

**Held positions and edits:** leverage and capital edits are not frozen into a held position — the next acting tick re-sizes what you already hold, on `live` with a real order. Plain TP/SL/time-stop edits apply to the next entry only; a held position keeps its frozen contract unless `apply_to_position` is passed and confirmed, and clearing the LAST trigger mid-hold retires the persisted policy next tick. Warn before any of these over a held live position.

**Consent flags are never yours to set.** `allow_same_market`, `allow_unverified_cohort`, `allow_existing_position`, and `cohort_crash_policy: "flatten"` come only from the user's explicit yes to the named risk — relay the refusal or approval card, ask, then retry with the flag (the four typed consent refusals are defined in §"Errors").

**Live transitions:** escalating into `live` disarms the strategy (explicit resume required); downgrading a HELD live strategy is refused (typed `strategy_live_position_held`; §"Errors"), and `force` leaves the position unmanaged without ever resuming management — offer flattening first and never suggest `force` casually.

**On Polymarket a flip is two sequential legs** — a SELL that fails leaves the OLD side fully exposed against the very view it reversed against (Hyperliquid nets in one order). **A live level rule is a standing reconciler:** it re-enters over the user's deliberate manual close while its condition holds — pause BEFORE intervening by hand. Pausing a signal auto-pauses every referencing strategy and `signal_resume` does NOT re-enable them — warn before pausing a referenced signal.
...
| Ask | Recipe |
| --- | --- |
| "paper trade the golden cross on BTC" | the signal first, then `strategy_create` with defaults — conviction mode/scale, fixed $10,000, UNPINNED unless a venue is named — created disabled: read the spec back in one block, offer `backtest_run`, ask before arming. |
| "trade my signal on Hyperliquid BTC / this Polymarket market" | `strategy_create` with the market pinned (`coin`, or `condition_id` + `long_outcome` — the inversion lives in exactly ONE of topic / `long_outcome`); a pinned create's run mode defaults to `paper` (echoed in `warnings[]`) — disclose it. |
| "paper trade a pair with no venue market" (any exchange+symbol the metrics plane serves) | `strategy_create` pinned `{venue: "market_data", exchange, symbol}` (e.g. BINANCE/BTCUSDT) — observe/paper only, leverage stays 1, `dry_run`/`live` refuse typed; going live = recreate pinned to a real venue. |
| "how is my strategy doing?" | `strategy_show` by slug; unnamed → `strategy_list` first (there is no most-recent default). Reply: P&L · trades · vs benchmark · next decision condition in words · chart link. |
| "stop it" / "pause it" | `strategy_pause` — never remove, never the signal; relay the `held_position` note verbatim. |
| "go live" / "arm it" | only on an explicit yes: confirm the wallet, `strategy_edit` run_mode (escalating to live DISARMS), then `strategy_resume` — a capital confirmation. |
| "never short" / one-sided | sizer mode `single_sided` — the only mode that never takes the other side. |
| "tighten the stop / change TP" | `strategy_edit` tp/sl — next entry only; a held position changes only via `apply_to_position` on the user's explicit confirmation. An "r/r > 2"-style ask is an agent-side translation — compute `tp = k × sl` and pass plain fractions; no ratio field exists. |
| "backtest it" | saved slug → `backtest_run`; your own unsaved idea → `backtest_spec` with an inline candidate (the research skill via `skill_read`); a registry strategy template → `package_try`, the marketplace funnel's default (it backtests the tuned candidate and mints the install token). |
...
- `close_only` blocks a **one-step flip**, not a reversal (the intro's warning). On the text lane every accepted event is fresh, so re-entry is immediate, as in the example; on the metric lane a *persisting* rule direction is deduped after the close, so re-entry waits for the direction to change anew.
...
Check `system_status` (above), confirm the prerequisites for the requested run-mode and signal kind, and route any missing setup to the user. For `observe`/`paper` you can proceed once the signal's inputs exist; for `dry_run`/`live` confirm the wallet is paired.
...
Strategies execute only under the daemon. If `system_status` showed it down, route the user to `om service install` then `om service start` (run as a service, not foreground, so decisions land in `runner.log`). The agent cannot start it.
...
- **`strategy_edit` has an edit/apply split for managed exits:** plain `tp` / `sl` / `time_stop_secs` edits update the spec for the next entry/flip only; an already-held position keeps its frozen active contract unless the edit carries `apply_to_position` on the user's confirmed yes. `clear_tp` / `clear_sl` / `clear_time_stop` unset a trigger (spec-only; refused with `apply_to_position`) — clearing the LAST trigger while a position is held retires the persisted policy on the next tick (native brackets canceled, position left to the signal), so warn the user before a full clear mid-hold.
...
- **A `strategy_edit` to `run_mode: live`** never starts live trading on its own: escalating a strategy from a non-live mode **into `live` disarms it** (persists `enabled=false`), so you must explicitly **`strategy_resume`** to begin live trading. This is scoped to the escalation transition — tuning an already-live strategy (a label/TP/`min_confidence` edit, or re-passing `run_mode: live`) leaves `enabled` untouched and does NOT halt a running live strategy; de-escalating to observe/dry_run also leaves it enabled. Tell the user the strategy is armed for live but dormant until they resume it. On a `market_data` pin the escalation itself is refused typed (`strategy_market_paper_only`) — going live means recreating pinned to a real venue.
...
The rest of the follow-list is the intro's guardrail block: status first, let-the-run-mode-default, derive-disclose (no sizing interrogation), create-disabled-ask-before-arming.
...
The rest of the avoid-list is the intro's guardrail block: no auto-arm, no secrets or wallet pairing, no unprompted `live`.
...
**Approval cards:** `strategy_remove` and `strategy_paper_reset` raise cards; `strategy_create`/`strategy_edit`/`strategy_resume` card on the args-gated shapes (live/enabled combinations and the consent flags — the card IS the consent on chat surfaces). A declined card is the user's no — never retry it or route around it.
...
```bash
om status --format json                  # the system_status read
