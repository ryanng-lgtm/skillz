...
5. **Ask where it reports.** Do not infer the channel or topic from the plan's subject. Ask the user to paste the destination — channel, and topic if the work belongs to one — and if the plan splits across areas, ask once for each. Then resolve what they paste to a canonical resource string with `session_grants` (and `room_topic_list` for the topic) and write that string into the brief, not the display name. Ask this while writing, never at 3am.
6. **Pick the trigger** (see table below).
